...
| `audit_card.shipped.length >= 3` AND `carry_over == []` AND `len(set(audit_card.shipped[*].assigneeId)) == 1` AND `audit_card.dropped.length == 0` AND `backlog_candidates` empty (or all Medium/Low priority) | Q1 of scope block still asked, but default switches to *"Defer to offline touch-base with owner (Recommended)"* with a 3-option lock (defer / park-this-cycle / specify-instead — see SKILL § 4.2). Q2–Q5 skipped + logged with reason `"ritual close-out — single owner consistent shipping, no signal for new scope"`; `scope_decisions.ritual = true` set on the project. |
