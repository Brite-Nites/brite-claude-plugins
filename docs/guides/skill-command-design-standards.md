...
- **[GATE] A skill that invokes an MCP tool declares `allowed-tools`.** A `SKILL.md` whose body references a fully-qualified `mcp__…` tool must declare a least-privilege `allowed-tools` — a skill with none inherits unrestricted tool access. Pure knowledge/reference skills that invoke no tool are **exempt** (declaring it would be noise); override a documentation *mention* (not an invocation) with a non-silent `# lint:no-mcp-invocation <reason>` marker. *R8, BC-16387 ([ADR-042](../decisions/042-skill-allowed-tools-mcp-coverage.md)) — a net-new gate rule, NOT part of the completed R2–R6 ratchet. **Skills only** (like R1 is command-only): the changed-set diff-gate is commands-only and never sees skills, so R8 is enforced full-surface by `eval_gate.py --structural` alone (mirrors R4). The skill surface is clean at flip time (0 findings) → no `docs/structural-lint-debt.md` rows; its value is forward-only. Commands that invoke MCP without `allowed-tools` are deferred (many name `mcp__…` tools only to specify subagent dispatch — a separate triage).*
