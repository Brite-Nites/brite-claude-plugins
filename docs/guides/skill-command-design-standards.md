...
- **[GATE] File references one level deep; TOC on any reference file > 100 lines.** Chained refs get partially read. *Promoted R4, BC-13217 (ratchet 5/5): the lint gates the chain-depth core — a bundled reference that itself references a further bundled file blocks; the "TOC > 100 lines" guidance stays reviewer advice. This is the final flip — the BC-12700 bullet-#2 ratchet (R2–R6 → blocking, per-rule) is complete.*
...
- **[GATE] Fully-qualified MCP tool names** (`mcp__plugin_<plugin>_<server>__*`) — *promoted R5, BC-13214 (ratchet 2/5)*; **[GATE] `${CLAUDE_PLUGIN_ROOT}` / `${CLAUDE_SKILL_DIR}` paths** (never hardcoded/absolute/backslash) — *promoted R6, BC-13215 (ratchet 3/5)*.
- **[GATE] A spec that invokes an MCP tool declares `allowed-tools`.** A `SKILL.md` **or command** whose body references a fully-qualified `mcp__…` tool must declare a least-privilege `allowed-tools` — a spec with none inherits unrestricted tool access. Pure knowledge/reference specs that invoke no tool are **exempt** (declaring it would be noise); override a documentation *mention* (not an invocation) with a non-silent `# lint:no-mcp-invocation <reason>` marker. *R8, BC-16387 ([ADR-042](../decisions/042-skill-allowed-tools-mcp-coverage.md)) — a net-new gate rule, NOT part of the completed R2–R6 ratchet, enforced full-surface by `eval_gate.py --structural` (the changed-set diff-gate is commands-only and never sees skills). Shipped skills-only; **extended to commands in BC-16865** ([ADR-042 § Amendment 2026-07-29](../decisions/042-skill-allowed-tools-mcp-coverage.md)) after a triage found all 16 MCP-referencing commands to be genuine orchestrator invocations rather than the expected mix with subagent tool-specs. The skill surface was clean at flip time (0 findings); the 16 command gaps are grandfathered as `(file, R8-allowed-tools-required)` rows in `docs/structural-lint-debt.md`, so the value on both surfaces is forward-only.*
