...
2. **A per-rule structural-debt list**: `docs/structural-lint-debt.md`, machine-read, rows keyed **`(file, rule)`** — a file can be grandfathered for one rule while still gated on every other. Schema `| file | rule | reason | added | baseline |`.
   - **Baseline** (R2-body-too-long rows only): pins the grandfathered body line count. The gate suppresses only while the body stays ≤ baseline; growth past it blocks. A plain file-level exemption would mute the size rule precisely on the biggest files — the rule's core failure mode.
   ...
   - **Malformed rows are loud**: a non-integer baseline, a baseline on a non-R2 rule, a missing rule id, or a duplicate key is a PROBLEM (gate red) and never suppresses — degenerate input must not silently become an exemption.
