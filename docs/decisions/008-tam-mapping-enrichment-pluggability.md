...
> **Interim deviation — CLOSED (default-flip executed 2026-07-09, BC-16888).** From BC-6170 → BC-13165 the order above ran as a "choice-now-default-later" interim: `brite_mcp` was an operative opt-in but removed from auto-detect. The engine-maturity sign-off gating the flip is recorded on BC-16888 (operator decision, 2026-07-08), and step 1 is restored — **auto-detect now resolves `brite_mcp` first** when the brite-enrichment MCP is registered. Immaturity risk stays bounded by the unchanged fail-open fall-through (`bulk_enrich` unreachable → `blitz_waterfall`, logged). Bulk-path robustness (BC-13096) and throughput (BC-6322) continue as independent hardening tracks, no longer gating the default.
...
**Future work.**
- ~~When BC-5538 ships, flip the auto-detect order so `brite_mcp` is preferred and the CLI fallback is removed from auto-detect (but remains available as explicit config).~~ **Shipped as an operative opt-in (BC-6170 → BC-13165):** `brite_mcp` is now a **working selectable opt-in** in both `list-building` and `tam-mapping`, routed through the `bulk_enrich` bulk door (ADR-017, never a single-call loop). The **config-invariant pair is now satisfied:** (1) the published `plugin_marketing_enrichment` server is pinned to a `bulk_enrich` SHA on Railway (BC-5316 / BC-13016), and (2) the skills' `allowed-tools` grant landed (BC-13165) — keep the two in lockstep going forward (rolling the pin back to a SHA without `bulk_enrich` silently disables the opt-in; it degrades to the `blitz_waterfall` fall-through, not an error). **Default-flip EXECUTED 2026-07-09 (BC-16888):** step 1 of the Unset resolution order restored — `brite_mcp` is auto-preferred when the brite-enrichment MCP is registered; the interim-deviation note above now carries the closure record. The engine-maturity sign-off was recorded as an operator decision (2026-07-08, on BC-16888); bulk-path robustness (BC-13096) and throughput (BC-6322) remain open as independent hardening tracks that no longer gate the default.
