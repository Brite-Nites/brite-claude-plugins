...
- **Land handbook back-pointer** — Create `handbook/marketing/go-to-market/campaign-rules.md` as a back-pointer to ADR-020. Update `campaign-lifecycle.md` § 2 (Touchpoint Frequency) to cite ADR-020 as the canonical machine-enforced statement, keeping the operator-facing prose but linking to the ADR for the precise rule text + exceptions. Handbook PR separate from this work.
...
- **Planning-time enforcement** — Extend [BC-10191](https://linear.app/brite-nites/issue/BC-10191) (`offer-catalog`) and [BC-10192](https://linear.app/brite-nites/issue/BC-10192) (`campaign-calendar`) to surface ADR-020 violations at planning time, not just launch time.
