...
- A behavioral eval **fixtures inputs → runs the command's emit mode → asserts on the produced artifacts** (JSON-schema + key-field assertions + golden-file compare). It does **not** assert on printed preview text.
...
- A **debt list** of un-eval'd existing commands/skills is tracked (Phase 2 backlog).
