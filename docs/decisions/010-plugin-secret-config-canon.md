...
- Preflight: requires `jq` on PATH + a vault session — either `BW_SESSION` exported by the caller, or (opt-in, 2026-07-29) minted by the wrapper itself when `BW_SESSION` is absent/stale: it reads the master password from the macOS Keychain item `bw-master` and runs `bw unlock --raw --passwordenv BW_PASSWORD`. Provision per machine with `security add-generic-password -U -a "$USER" -s bw-master -w`; without the item, behavior is the original fail-closed exit. `bw status` must report `unlocked` after at most one mint attempt per gap; any preflight failure exits non-zero with a one-line stderr remediation that includes the provisioning hint. **Trust trade-off (accepted):** once the Keychain item exists and `security` access is granted, any process running as the user can open the vault — the same trust level as browser-extension autofill. Accepted to eliminate the vault-lock-mid-session recovery tax and the "MCP spawn fails because the launching shell had no BW_SESSION" failure mode. The minted session flows through the same pre-`exec` `unset BW_SESSION` as a caller-provided one. Because this path carries the **master** password (strictly worse to leak than a session token — a session dies on `bw lock`, the master password does not), it resolves `security` and `bw` to absolute paths instead of inheriting PATH, and each resolved path must pass a trust check: either a known install path (`/opt/homebrew/bin/bw`, `/usr/local/bin/bw`, `/usr/bin/bw`, `/usr/bin/security`), or any path on a **safe path**: its directory is mode 0700 and owned by us, and every ancestor is owned by us or root and is not group/other-writable unless it is sticky. The known-install list is a **narrowing filter, never a pass** — a listed path still has to clear a directory check (owned by us or root, not group/other-writable, ancestors likewise); it is exempt only from the 0700 requirement, which a 0755 system install cannot meet. An earlier cut returned success on a bare string match, making the one branch that meant "known good" the only branch that verified nothing — which mattered because Homebrew makes `/usr/local/bin` and `/opt/homebrew/bin` user-owned and commonly group-writable, exactly the shape these checks exist to reject (corrected 2026-07-31, before merge). Symlinked binaries are followed to their target and **both ends are checked**: the link's own directory and the target's. Checking only the link says nothing about where it points, and Homebrew legitimately symlinks its bin entries into `../Cellar`, so the list is matched against either end while the directory checks apply to both. **The check itself must not be steerable by what it defends against.** It leans on `ls`, `find` and `readlink`, all PATH-resolved, so an attacker able to seed PATH could substitute `ls`, have every directory report `drwx------`, and make every rule above decorative (found at Greptile review 7; the hole had been present since the first hardening pass). `_self_unlock` therefore pins `PATH` to `/usr/bin:/bin:/usr/sbin:/sbin` for its whole duration — sudo's `secure_path` move — and restores the caller's PATH before returning, so the wrapper's later bare `bw`/`jq` calls stay in the deferred scope above rather than being narrowed by a side effect. `dirname`, `basename` and `cut` were replaced with bash parameter expansion, which cannot be substituted at all; shrinking the external surface to three is what makes the pin a claim worth trusting. One consequence, deliberate: a pinned PATH cannot contain the Homebrew prefixes (they are the group-writable case `system` exists to reject), so `bw` **discovery** no longer consults PATH either — the known-install list is walked directly and the first entry that is both present and trusted wins. An install elsewhere is still reachable through `BW_RUN_BW_BIN`, deliberately rather than by accident. Finally, **the binary itself is checked, not only what contains it** (Greptile review 8): every rule above constrains a directory, and a root-owned 0755 `/usr/local/bin` passes while the `bw` inside it is group-writable or owned by someone else — whoever can write that file chooses what receives the master password. The resolved target must therefore be owned by us or root and not group/other-writable. Consequence worth stating: a group-writable `bw` is refused outright, `ssh`-StrictModes style, because group membership cannot be checked portably. macOS's default umask of 022 makes this a non-event for Homebrew and system installs; where it does bite, self-unlock declines and the original `bw unlock` + exported `BW_SESSION` path still works. Ownership is checked with `[ -O ]`, not inferred — an earlier cut of this reasoned that a mode-0700 directory can only be traversed by its owner, so reaching the binary proved we were that owner. That inference fails silently for root, which bypasses permission checks, so running as root an attacker-owned 0700 directory would have passed (corrected 2026-07-31, before merge). Ancestors are walked because mode 0700 stops a co-tenant writing *into* a directory but not renaming the directory itself, which needs only write on its parent; one attacker-writable non-sticky ancestor and the whole thing is swappable. The sticky exemption is what makes a shared parent like `/tmp` safe — sticky means only an entry's own owner may rename or delete it — and it exempts the write-bit test only, never the ownership test, since a sticky directory's owner can still rename entries inside it. Paths are resolved with `pwd -P` before the walk so it sees real directories rather than symlinks, whose modes are meaningless. Relative paths are rejected outright. The `BW_RUN_BW_BIN` / `BW_RUN_SECURITY_BIN` overrides get the **same** check rather than bypassing it: an env var an attacker can also set is not an exemption from the check it would otherwise defeat (the first cut of this ADR granted exactly that exemption; corrected 2026-07-31 before merge). If either binary fails to resolve to a trusted path the mint is skipped entirely and the master password is never read. The Keychain lookup carries the same `-a` account as the provisioning command, so a same-service item belonging to another account cannot win it. At most **one** mint attempt per process. A self-minted session is **never exported**: it is passed to the already-trusted binary one invocation at a time, so neither a PATH-substituted `bw` nor a PATH-substituted `jq` downstream of it ever holds a live vault token. Minting is the case where the wrapper brings a token into existence that otherwise would not exist, so it is the case that must not leak one. Scope line, stated so it is a decision and not an oversight: a **caller-supplied** `BW_SESSION` is already exported in the wrapper's environment by whoever launched it, and the `status`/`list`/`get` calls still resolve through PATH and still receive it — pre-existing behavior, unchanged here, and narrowing it is a separate proposal. (An earlier draft of this line claimed the self-unlock "neither introduces nor worsens" that exposure. Not quite: before self-unlock, an absent session simply exited, so there was no token to leak — minting one and exporting it would have been a genuinely new exposure, narrow but real. Corrected 2026-07-31 by not exporting it, before merge.)
...
- **Accepted residual — PATH-resolved `jq` sees fetched item data (2026-07-31).** The batch path pipes `bw list items` output, which contains plaintext item passwords, to a bare PATH-resolved `jq`. A substituted `jq` can read it. Recorded as accepted, not overlooked. The distinction that decides it: `unset BW_SESSION` above is a boundary the wrapper *asserts* — the wrapped process is meant never to hold the vault token — so leaking that token sideways to `jq` defeats a stated guarantee and was fixed. The per-vendor secrets are the opposite: handing them to the wrapped process is the entire job. The wrapper's terminal act is `exec "$@"`, and every registered server names its command bare (`npx`, `node`, `node`, `uvx` in `plugins/marketing/.mcp.json`), so an attacker holding PATH substitutes the exec target and receives every secret directly. Hardening `jq` would guard, for three lines, a value deliberately given away on the fourth. It is not a boundary, so it does not get boundary machinery. Revisit only as part of a decision to trust-resolve the exec target itself — which the current `.mcp.json` shape cannot support unchanged, and which is a separate proposal. Raised by Greptile at 4/5 on PR #565 and merged over with this reasoning on the record.
