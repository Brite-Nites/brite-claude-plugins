...
- **One residual is accepted, not overlooked.** Findings identical in file, code *and* message cannot be told apart, so such a finding can be **relocated** within its file without tripping the ratchet — most plausibly `F541` (`f-string without any placeholders`), whose message names no symbol. It cannot **add** debt: the count is pinned, so a file's total for that message can only fall. Closing it means putting position in the key, and both forms cost more than they buy — line numbers churn the baseline on every unrelated edit above a finding, and a source-text anchor churns on reformatting. ADR-034's R2 baseline makes exactly this trade: it pins a body's line *count*, not which lines.
