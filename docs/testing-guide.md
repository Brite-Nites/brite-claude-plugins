...
| T6.26 | `cdr-compliance-reviewer` | `/workflows:review` (when project CLAUDE.md has `## Company Context` with `handbook-library`, CLAUDE.md enables, or `comprehensive` depth) | Until the CDR INDEX retrieval mechanism is restored, the agent always skips CDR loading via step 4 of the Loading Protocol with `CDR Compliance: N/A (CDR INDEX unavailable)`, then continues to URL Resolution (step 4 explicitly does NOT stop). The two earlier short-circuit paths terminate before URL Resolution per their "Stop here." directive — step 2 emits `CDR Compliance: N/A (no handbook configured)` when no `handbook-library` line is present; step 3 emits `CDR Compliance: N/A (invalid handbook-library)` when the value does not match the `/org/repo` pattern. |
