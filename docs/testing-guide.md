...
| T6.26 | `cdr-compliance-reviewer` | `/workflows:review` (when project CLAUDE.md has `## Company Context` with `handbook-library`, CLAUDE.md enables, or `comprehensive` depth) | Until the CDR INDEX retrieval mechanism is restored, the agent always skips CDR loading with `CDR Compliance: N/A (CDR INDEX unavailable)` and proceeds to URL Resolution. When no `handbook-library` line is present in the `## Company Context` block: skip with `CDR Compliance: N/A (no handbook configured)`. When the `handbook-library` value does not match the `/org/repo` pattern (e.g., missing leading slash): skip with `CDR Compliance: N/A (invalid handbook-library)`. URL Resolution Check still runs against the diff in all three skip paths. |
