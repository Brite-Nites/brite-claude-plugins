...
- The skill-registry row was resolved by BC-16683 (2026-07-07): `shared/hooks/skills-registry.json` is a deliberate superset (ADR-007 §3.5 amendment), so each absent entry now carries an `absent_reason` marker and `test_registry_does_not_reference_missing_sf_skill_directories` was re-scoped to fail only on unmarked drift and un-quarantined.
