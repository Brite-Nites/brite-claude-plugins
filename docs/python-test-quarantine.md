...
| `plugins/revops/tests/hooks/test_apex_validators.py` | `TestApexPostToolBad::test_bad_cls_scores_lower` | Expects a "bad" Apex fixture (`BadService.cls` — SOQL-in-loop, `ArrayList` Java-type hallucination, hardcoded password logging, no `with sharing`) to score lower than a "good" fixture (`AccountService.cls`). Both score **90/90 "Excellent"** identically. **Production bug, not test rot.** | `skills/sf-apex/hooks/scripts/post-tool-validate.py` (PHASE 1.5, ~lines 76-97) appends `llm_pattern_validator.py`'s findings (e.g. the 2 CRITICAL "Java type ArrayList does not exist" issues on `BadService.cls`) to the *displayed* issues list only — it never feeds them back into `custom_score`/`custom_scores`, which come solely from `validate_apex.py`'s static rubric. So a file can show CRITICAL findings in its issues list while still reporting a perfect numeric score. Confirmed this is the advisory `post-tool-validate.py` hook (non-blocking, docstring says so explicitly) — it does not gate/block Salesforce operations, so it is a scoring-accuracy bug, not a bypassed security gate. Fix is separate — filed as a follow-up, not done in BC-16289. | 2026-07-06 |
