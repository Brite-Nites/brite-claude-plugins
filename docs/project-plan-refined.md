...
> **Note (2026-05-19)**: The shipped command at `plugins/marketing/commands/plan-campaign.md` is the authoritative spec. This section is the original refined-plan spec — preserved for audit trail. Key deltas vs the shipped command: (a) `allowed-tools` uses the `Skill` tool to compose `/revops:create-sf-campaign` (BC-8717) and `/revops:update-sf-campaign-status` (BC-8723) — the MCP write tools listed in Step 2 below do NOT exist (both were respec'd to slash commands on 2026-05-19); (b) sub-issue count is **8 standard + 2 optional** (not 7+2); (c) validation tuple uses real canonicals (e.g. `municipalities × parks-rec-director × parks-bond × M05`) — `public-works-director` and `free-rop-audit` referenced below do not exist in the data layer.
