...
3. **Populate `_fetched_issues` cache from snapshot (BC-5902).** If `manifest_entry.issue_snapshot` is present, write it to `state.projects[i]._fetched_issues[manifest_entry.issue_id]` (mapped to whichever project owns the mutation via `row.source_project`; fall back to the top-level `_fetched_issues` map keyed by `issue_id` if the state shape doesn't carry per-project caches for non-project-scoped mutations). § 3's pre-flight reuse ladder step 1 reads this cache for equality comparisons against `after.cycleId`/`after.stateType`/`after.assigneeId`/`after.labelIds`, eliminating the redundant `get_issue` per cycle-path row that would otherwise fire in § 3.
...
The count of `AskUserQuestion` group-approval calls for the **regular decision-path groups** (step 4) equals the number of distinct non-empty decision-path groups in the checkpoint — this is the literal AC #3 assertion. Pre-groups (0/1/2/3) are per-row by necessity — each CQ3 parse error, conflict, or live gate failure requires individual resolution because merging them into a single batch prompt would lose the per-row context the user needs to decide. Fix-now and Override follow-ups inside regular groups are also per-row conditional and not part of the AC #3 regular-group count. When tallying AC #3 evidence, count only the prompts fired in step 4. Pre-group 2 (Preflight errors) is also per-row because each fetch failure needs individual Retry/Override/Drop resolution.
