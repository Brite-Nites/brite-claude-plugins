...
- **Phase 1 quality-flags render has a 10-flag threshold, not a word cap.** `commands/weekly.md § 1.6 item 5` renders one line per flag when `≤10 flagged total`; at `>10` it renders a single aggregation line with top-3 projects by flag density + top-3 check types (absolute count, ties broken alphabetically). `audit.json` always holds the full per-flag record regardless of which render path fires — the render is a display decision, not a data-loss decision. Origin: BC-5870 (W17 dogfood surfaced 75 flags silently aggregated against the literal per-flag spec).
