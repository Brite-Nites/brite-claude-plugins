...
> **Context cascade**: This step loads Tier 1+2 context. Tier 3 CDR INDEX retrieval is currently unavailable (see Step 4); when the mechanism is restored it will load on-demand. See `docs/designs/BRI-2006-context-loading-cascade.md` for the full cascade spec.
...
4. **CDR INDEX (handbook)** — Skipped. The CDR INDEX retrieval mechanism is currently unavailable (no handbook-query path is wired). Log: "CDR INDEX check skipped — retrieval mechanism unavailable" (Decision Log format) and proceed. Omit the `CDR alignment` and `CDR exceptions` fields from the plan template when there is nothing to record.
...
- CDR check is advisory, not blocking. The retrieval mechanism is currently unavailable; the check is always skipped — log why and proceed with planning.
