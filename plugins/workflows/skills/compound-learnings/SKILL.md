...
CDR cross-reference against the Brite Handbook is currently unavailable (no handbook-query mechanism is wired). Set the **Precedent Referenced** field to `None — first time encountering this pattern` for all traces, then proceed to ADR cross-reference. ADR cross-reference (Section 2e) still runs and may populate the field.
...
- Check output against anti-slop guardrails (see `_shared/anti-slop-guardrails.md`). Relevant patterns: C1-C2 (ephemeral knowledge in CLAUDE.md, CLAUDE.md bloat past ~100 lines). Violations cap Adherence score at 3 in rubric evaluation.
