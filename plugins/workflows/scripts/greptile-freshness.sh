...
#   FRESH_PASS  fresh review (max(verdict_ts, review_ts, edited_ts) > trigger) with score == 5
...
# NOTE: --edited-ts is appended LAST so the positional contract of the older
# args is unchanged (a caller that omits it behaves exactly as before).
python3 - "$TRIGGER" "$NOW" "$DEADLINE" "$VERDICT_TS" "$SCORE" "$REVIEW_TS" "$EDITED_TS" <<'PY'
import sys, datetime
...
trigger, now, deadline, vts, score, rts, ets = (sys.argv[1:8] + [""] * 7)[:7]
...
# but two other signals do advance: the head-SHA check-run's completed_at
# (review-ts, BC-16924) and the comment's own updated_at (edited-ts, BC-12580 /
# BC-17025). review-ts alone has a blind spot — when Greptile auto-reviews the
# push BEFORE the re-review is requested, its check-run completes pre-trigger and
# the in-place edit that follows is invisible. edited-ts closes that at source.
# Use whichever signal is latest; any one of the three alone still works.
