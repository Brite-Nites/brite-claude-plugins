...
- **Lead-export scoping and row caps are silently wrong on three of four paths** (verified 2026-07-23 against workspace 13 campaign 74 — a 531-lead campaign; BC-17215). Every call below returned `success: true`. None error, none warn:

  | Call | Returned | Behavior |
  |---|---|---|
  | `export_leads_csv(campaign_id=74)` | 3,000 | **`campaign_id` ignored** — despite being documented "Filter by campaign ID" |
  | `bulk_export(campaign_leads, campaign_id=74, format=csv, max_items=1000)` | 150 | silently truncated |
  | `bulk_export(campaign_leads, campaign_id=74, format=csv, max_items=20000)` | 531 | correct |
  | `bulk_count(campaign_leads, campaign_id=74)` | 531 | correct |
  | `list_leads` (any filter combination) | 16,502 | no `campaign_id` parameter exists on this tool |

  Three rules follow. **(1) Never use `export_leads_csv` for a campaign-scoped pull.** Its `campaign_id` does nothing, so you get an unscoped workspace slice capped at 3,000 rows. A 531-lead campaign returning 3,000 rules out a cap explanation — the filter genuinely does not apply. **(2) `max_items` is a yield knob, not a safety limit.** Observed output fits `(max_items / 100) × 15` — roughly **15% of what you ask for** — so set it to **~7× the rows you actually want**. **(3) Always assert the count.** `bulk_count(resource="campaign_leads", campaign_id=N)` is the oracle (so is `bulk_export(..., format="summary").total_items`); compare the exported `row_count` against it and hard-fail on mismatch rather than warn. This matters most where the export feeds a write: a silent wrong-set on a Salesforce backfill loads thousands of unrelated leads as real Contacts and Accounts, which is the expensive direction to undo. Compounding it, leads return newest-first — so when the target campaign was the most recent load, even an **unscoped** pull's first page looks like the right campaign, and spot-checking confirms nothing.

  Correct campaign-scoped export: `bulk_export(resource="campaign_leads", campaign_id=N, format="csv", max_items=<~7× expected rows>)`.
...
`2026-07-23` — BC-17215: appended 1 §Known gotchas bullet (lead-export scoping + row caps silently wrong on three of four paths). Verified live, read-only, against `emailbison-personal` workspace 13 campaign 74 ("FY26, M07 | Wineries | Personal Emails | All ESPs", `total_leads: 531`): `export_leads_csv(campaign_id=74)` → 3,000 rows (filter ignored); `bulk_export(campaign_leads, 74, csv, max_items=1000)` → 150; same at `max_items=20000` → 531; `bulk_count(campaign_leads, 74)` → 531; `list_leads` → 16,502 workspace-wide (tool exposes no `campaign_id`). All returned `success: true`. The observed `(max_items / 100) × 15` yield is consistent with the previously-documented hardcoded `per_page=15` (Sx-10, BC-6298). Surfaced while de-risking the BC-17215 winery backfill, where an unscoped export would have written ~3,000 unrelated leads to production Salesforce as Contacts + Accounts.
