...
**Cherry-picking individual tools via `--tools`.** Upstream's `--tools` flag (`packages/mcp/src/index.ts:97` — `multiple: true, delimiter: ','`) composes with `--toolsets`: you can enable a narrow toolset (e.g. `data`) and cherry-pick individual tools from other toolsets without exposing their parent toolset. Worked example — [BC-2725](https://linear.app/brite-nites/issue/BC-2725/) `lead-routing` will need `assign_permission_set` (from the `users` toolset) for conditional territory-based permset grants, but nothing else from `users`. The registration becomes `--toolsets data --tools assign_permission_set`, keeping the exposed surface at 4 tools instead of the dozen-plus that `--toolsets users` would add. When a consumer skill motivates the cherry-pick, add it here alongside a short rationale and bump the plugin version; do not enable a whole toolset to get one tool.

**Telemetry is disabled (`--no-telemetry`).** Upstream defaults telemetry ON and sends session/CLI identifiers + `NODE_ENV` + internal-host detection via Application Insights (`packages/mcp/src/telemetry.ts` — `APP_INSIGHTS_KEY` populated at publish time). Runtime toggle is the `--no-telemetry` CLI flag; there is no env-var equivalent. We opt out for the plugin-scoped registration so no per-dev metadata leaves the developer's machine.
...
- **Directory parameter trap — every `@salesforce/mcp` data/metadata tool requires a `directory` parameter pointing at a real SFDX project path on disk** (discoverable via `sfdx-project.json`). Marketing consumer skills ([BC-2717](https://linear.app/brite-nites/issue/BC-2717/), [BC-2720](https://linear.app/brite-nites/issue/BC-2720/), [BC-2725](https://linear.app/brite-nites/issue/BC-2725/), [BC-2727](https://linear.app/brite-nites/issue/BC-2727/), [BC-2728](https://linear.app/brite-nites/issue/BC-2728/)) called from a non-SFDX cwd (e.g., `britenites-claude-plugins/` itself, or an unrelated project directory) will fail with a path-not-found error — the MCP server starts fine, but per-call tool invocations reject. Mitigations: consumer skills should either assert that `pwd` contains an SFDX project before calling the MCP, or pass an explicit `directory` pointing at a local `brite-salesforce/` checkout. A server-recognized sentinel or zero-directory fallback is a separate spike when the first consumer skill ships; for now, document and route around.
