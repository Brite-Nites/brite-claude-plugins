...
- **[BC-5830](https://linear.app/brite-nites/issue/BC-5830) `campaign-debrief`** — feedback loop: `campaign-debrief` writes `transferable_note` YAML values to `docs/campaigns/{entity}/learnings.md`; this skill's §3 ITERATE Step 3.5 reads those values and populates the appended Results Log row's **Transferable Insight** column with `[from debrief: ...]` provenance markers, closing the loop between batch execution and matrix evolution while preserving the body-matrix append-only invariant. **Entity-slug normalization (post-BC-8719):** both skills now write to the same short-form `docs/campaigns/{entity}/` directory (`{entity}` ∈ {`nites`, `supply`, `labs`}). The legacy long-form layout (`docs/campaigns/brite-{entity}/`) is read-compatible via Step 3.5's backward-compat Glob fallback for one release cycle, after which it retires — operators with pre-BC-8719 artifacts should run `python3 plugins/marketing/scripts/migrate_entity_slugs.py` once.

**Does not own:**

- Campaign execution (that's `outbound-playbook` plus `launch-campaign` — BC-2722 pending).
- Copy generation (that's `email-copywriting`; MSPA only classifies messaging by experiment verdict, never writes subject lines or bodies).
...
6. Run §3 ITERATE Step 3.5 — pull `transferable_note` values from `learnings.md` into the Results Log row's **Transferable Insight** column per Step 3.5's construction rule. Skip silently if absent.
7. Run §3 ITERATE Step 4 — append the Results Log row(s) to `docs/campaigns/{entity}/mmf-matrix.md` using the four fixed experiment verdict tokens only (column header stays `Verdict` per ADR-018); populate the Transferable Insight column where known.
...
**Handoff:** `campaign-debrief` receives the results log once the Transferable Insight column is populated on at least one row — transferable learnings flow to `docs/campaigns/{entity}/learnings.md` via that skill (short-form canonical post-BC-8719), and return via Step 3.5 on the next ITERATE run.
...
- **Do not fabricate experiment verdict labels.** Only `SUPER WORKS`, `KIND OF WORKS`, `DOESN'T WORK`, `DEFERRED`, `PENDING` are allowed in the matrix `Verdict` column (column header stays `Verdict` per ADR-018). Prose substitutes ("promising", "mediocre", "worth another shot", "pretty strong") are refused — §7 1-3.
...
- **`verdict-labels-only`** — Given any mode producing a matrix update (MAP first-batch Verdict column, ITERATE Results Log Verdict column), every experiment verdict cell reads exactly one of the five fixed tokens: `SUPER WORKS`, `KIND OF WORKS`, `DOESN'T WORK`, `DEFERRED`, `PENDING`. Scenario fails on any prose substitute ("promising", "mediocre", "decent", "worth another look").
...
- **`learnings-md-read-populates-transferable-insight`** — Given ITERATE invoked on Nites with an existing `docs/campaigns/nites/learnings.md` (short-form canonical, post-BC-8719) containing an entry where `campaign: spring-promo-2026-04-15`, `transferable: true`, and `transferable_note: "tier-2 venue angle outperforms tier-1 by 1.8x at the same spend"`, the post-ITERATE `docs/campaigns/nites/mmf-matrix.md` Results Log row whose `Experiment` column matches that campaign name carries `[from debrief: tier-2 venue angle outperforms tier-1 by 1.8x at the same spend]` in its Transferable Insight cell. If the operator drafted a Transferable Insight in Step 3, the debrief-sourced text is appended after `; `. The body-matrix rows are byte-for-byte unchanged (append-only invariant preserved). Scenario fails if the marker is missing, if the pulled text is paraphrased, if any body-matrix row was mutated, or if the provenance marker appears in the body-matrix `Notes` column. Requires a before/after diff of `docs/campaigns/nites/mmf-matrix.md`.
- **`learnings-md-legacy-long-form-read-warn`** — Given ITERATE invoked on Nites with NO `docs/campaigns/nites/learnings.md` (short-form Glob empty) but a pre-BC-8719 `docs/campaigns/brite-nites/learnings.md` present on disk (legacy long-form Glob hits), Step 3.5's backward-compat fallback reads the legacy long-form file, emits the literal `[BC-8719]` stderr warning exactly once with a pointer to `plugins/marketing/scripts/migrate_entity_slugs.py`, and populates the Results Log row's Transferable Insight column from the legacy file's `transferable_note`. The legacy file is NOT mutated by this run; the Write target is the short-form `docs/campaigns/nites/mmf-*.md` family. Scenario fails if the warning is missing, fires more than once, mutates the legacy file, or omits the `[from debrief:` provenance marker.
- **`learnings-md-absent-degrades-cleanly`** — Given ITERATE invoked on Nites with NO `learnings.md` file on disk at EITHER the short-form `docs/campaigns/nites/learnings.md` OR the legacy long-form `docs/campaigns/brite-nites/learnings.md` (both Globs return empty), Step 3.5 is skipped silently — no warning emitted to the operator, no halt, no error. Flow 2 continues to step 7 (Step 4 Results Log append) and produces `mmf-results-{N}.md` + matrix append + `mmf-batch-{N+1}.md` normally. The appended Results Log rows carry only whatever Transferable Insight the operator drafted in Step 3 (may be empty) — no `[from debrief: ...]` markers appear anywhere in `mmf-matrix.md`. Scenario fails if a warning message fires, if the run halts, or if a `[from debrief:` string appears anywhere in the matrix file.
