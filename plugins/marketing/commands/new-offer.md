...
The behavioral eval (BC-12702, ADR-028 § 5 — the **structure-first / LLM-judged** representative) drives the SAME deterministic builder this command delegates to (`canonicals_bootstrap.py offer`) against a **sandbox copy of a frozen fixture seed** — `canonicals_bootstrap.py --canonicals-dir <sandbox> offer …` — so the run is **side-effect-free**: no write to the real `data/canonicals/`, no Linear/EB/handbook write, no network. The eval-only harness `plugins/marketing/scripts/build_offer_emit.py` orchestrates this (copy seed → run the builder → run `lint_canonicals.py --canonicals-dir <sandbox>`) and emits a structural matrix (`offer-emit.json`).

The eval asserts the artifact's deterministic **STRUCTURE** — that the written canonical passes the full 19-check `lint_canonicals` contract (schema validity, kebab slugs, filename-stem==slug, status/posture enum, no duplicate offer slug, `target_personas` referential integrity) and the appended entry's `slug`/`display`/`status`/`posture` match the inputs — and explicitly **NOT** the operator/LLM-chosen content (which offer/posture/personas, the handbook-draft prose). That is the ADR-028 D2 structure-first cascade: a judgment-bearing command still passes *deterministically* on the per-PR gate. Eval data lives under `plugins/marketing/tests/eval/new-offer.*` + `new-offer-seed/`; harnesses are `scripts/eval/test_eval_harness.sh` + the builder suite `plugins/marketing/scripts/test_build_offer_emit.sh`, both wired into `validate.sh`.
