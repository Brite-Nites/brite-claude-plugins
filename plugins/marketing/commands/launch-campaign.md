...
1. **Materialize the cleaned lead file, then invoke the `salesforce-preload` skill** via the `Skill` tool:
...
2. **Consume the skill's result.** The skill returns (a) the **surviving lead set** — every row safely represented in Salesforce (net-new created, single match attached, or already-present duplicate), with **needs-review rows removed**; (b) **counts** (accounts created, contacts created, matched, multiple flagged, held); and (c) the **review-sidecar path** — the skill's own `-preload-review.csv`, which already holds every needs-review + flagged row.
   - The **surviving set replaces the cleaned lead set** that Phases 2–11 consume; update the run's working lead set and set `lead_count` to the surviving count (every downstream count check — Phase 4 chunking + step 9, Phase 6 attach — then uses this reduced count, exactly as it uses the Phase-1 cleaned count today).
...
3. **Record + outcome.** Append to the metadata JSON: `salesforce_preload: {"status": "complete", "instance": "personal", "target_org": "<org>", "accounts_created": N, "contacts_created": N, "matched": N, "multiple_flagged": N, "held": N, "planned_vs_created": [P, C], "sidecar_path": "<path>|null"}`, **and re-write the top-level `lead_count` to the surviving count from step 2** — otherwise a fresh-session resume (scratch state gone, metadata the only source) would keep the pre-reduction count. **Leave `last_completed_phase` at 1** — Phase 1b is a sub-phase of PRE-FLIGHT, so the integer breadcrumb stays monotonic (1 → 2 → …). If the skill **HALTED** (e.g. the entire list is "no company", or the operator aborted the write-gate), **record `salesforce_preload: {"status": "halted", ...}`** and **HALT the campaign** — nothing proceeds to Phase 2 or the EB upload. Recording a non-`complete` status is what lets a later resume detect the pre-load is unfinished (see the resume rule in § Inputs and § Launch metadata schema).
