...
9. **Attach the sending-identity tag to each campaign (BC-13863).** Push the `sending_identity` resolved in Phase 1 (`labs`/`supply`/`nites`) onto every campaign created or reused above, so downstream tooling — and BC-13864's identity-matched sender selection — can filter by it. Identity tags are **per-workspace** (the same identity has different tag ids in the commercial vs personal instances), so the id is resolved fresh per run — done up front in **Phase 1 step 10**, never hardcoded.
   ...
   a. **Use the identity tag id resolved + existence-verified in Phase 1 step 10.** Because resolution and the missing-tag HALT happen in pre-flight (before any EB state is created), a misconfigured workspace fails closed *before* step 6 creates campaigns — no orphaned, unrecorded campaigns. **Resume guard:** Phase 5 runs only on a real (tagging) launch — `--preview` skips it — so if `sending_identity` is null/absent here (e.g. the run resumed from a `--preview` breadcrumb, which records it null), resolve it now via the Phase 1 step 10 logic (prompt + tag-existence HALT) before attaching; never attach a null identity. This command only attaches the tag; creation is owned by the platform reconcile (BC-13861). See `email-infrastructure-orchestration-platform/docs/designs/email-bison-tag-behavior.md` for tag behavior.
