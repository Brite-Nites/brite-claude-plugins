...
2. **Enumerate this run's connected senders.** Page `list_sender_emails`, fail-closed. How many pools, and what filter each uses, depends on `sender_match_mode`: the **uniform** modes (`identity`, `all`) enumerate **one** pool attached to every campaign; the **per-bucket** modes (`esp`, `both`) enumerate **one pool per campaign cell**, each with that cell's ESP filter.
   ...
   **Per-bucket modes (`esp` / `both`):**
   ...
   i. **Empty-cell handling (never silent — surfaced + confirmed at User gate 7).** If a cell's pool from (g) is empty:
      - **`both` mode → fall back to identity-only for that cell.** Re-enumerate that cell with `tag_ids=[identity_tag_id]` alone (drop the ESP filter), verify it per the identity cross-check (2c), and use that identity pool for the cell — redirect-safe, and it keeps the cell's already-attached leads moving rather than stranding them. Record the cell in `sender_match_fallbacks` (step 8). Never applied silently: it is shown and confirmed at gate 7.
...
5. **User gate 7.** Ask via `AskUserQuestion` — the prompt depends on `sender_match_mode`.
   ...
   **Per-bucket (`esp` / `both`):**
   ...
   >
   > *(If step 2i found empty cells, list them here — never silent:)*
   > ⚠️ Cells with no ESP-matched senders:
   > - `{cell}` — **`both`: falling back to identity-only ({M} `{sending_identity}` senders)** ✅ redirect-safe
   > - `{cell}` — **`esp`: NO senders — will NOT send** unless you abort and fix coverage (or attach manually later)
   >
   > - Yes — attach each cell's matched pool{, apply the identity fallback for the empty `both` cells shown, and leave the empty `esp` cells unsent} (Recommended)
   > - Abort
