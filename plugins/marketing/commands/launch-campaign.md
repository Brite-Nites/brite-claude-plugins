...
- **CSV** — enriched lead file at `--csv`. Columns are resolved through the shared column mapper (`plugins/marketing/scripts/_shared/column_map.py`, BC-17213), so header spelling is flexible — `Company Domain` / `domain` / `Website`, `First Name` / `firstname`, `Company` / `Organization` / `company_name` all resolve (the Apollo / Serper / Clay / hand-built variants the mapper already handles). Required (each must resolve to some header): `email`, `first_name`, `domain`. Optional: `last_name`, `title`, `company`. Phase 1 step 1 runs the resolution, applies this command's own required set (which diverges from the mapper's — see there), and records the field→header map for Phases 2 and 4.
...
- **Launch metadata JSON** at `docs/campaigns/{short_entity}/{campaign-name}-{YYYY-MM-DD}.json` — written progressively, one phase at a time. On partial failure, the metadata file IS the breadcrumb — re-run picks up from `last_completed_phase + 1` manually. **Personal-instance exception (BC-17214):** Phase 1b (Salesforce pre-load) is a PRE-FLIGHT sub-phase that does **not** advance `last_completed_phase`; on the `emailbison-personal` instance a resume MUST check `salesforce_preload.status` and re-enter Phase 1b when it is absent or not `complete`, **before** advancing to `last_completed_phase + 1` — otherwise leads reach the EB upload with no pre-load and recreate the `gmail.com`-Account failure.
...
```json
{
  ...
  "lead_count": 127,
  "salesforce_preload": {"status": "skipped", "reason": "commercial"},
  "segmented": true,
  "segments": {
...
`salesforce_preload` (BC-17214) records the Phase 1b outcome. On `emailbison-b2b` it is `{"status": "skipped", "reason": "commercial"}` (as in the example above). On `emailbison-personal` it is `{"status": "complete", "instance": "personal", "target_org": "<org>", "accounts_created": N, "contacts_created": N, "matched": N, "multiple_flagged": N, "held": N, "planned_vs_created": [planned, created], "sidecar_path": "<path>|null", "surviving_csv_path": "<path>"}` (or `{"status": "preview", …}` under `--preview`, and `{"status": "halted", …}` on a halt). `surviving_csv_path` is the durable file every phase from 2 onward reads instead of the raw `--csv` — the source of truth for which rows proceed past the pre-load. It is tracked **separately from** `last_completed_phase` (which stays a 1–11 integer): Phase 1b is a PRE-FLIGHT sub-phase and keys its idempotent re-entry off `salesforce_preload.status`, not the integer breadcrumb. A resume on the personal instance therefore MUST re-enter Phase 1b whenever `salesforce_preload.status` is absent or not `complete`, before advancing past Phase 1 — the `last_completed_phase + 1` rule alone would skip it (see § Inputs).
...
- Phase 1 step 1 (resolved) / step 10 (written): `resolved_columns: {"email": "<header>", "first_name": "<header>", "domain": "<header>", "last_name"?: "<header>", "title"?: "<header>", "company"?: "<header>"}` — the canonical-field→CSV-header map from the shared column mapper (BC-17334). Resolved in step 1 and held in scratch state; step 10 writes it (the metadata file does not exist until then). The three required keys (`email`, `first_name`, `domain`) are always present (the run HALTs otherwise); optional keys appear only when that column resolved. Phase 1 step 2a (dedup), Phase 2 (email-type + domain parse), Phase 4 (lead-body build), and Phase 10 (local render) all read this instead of literal column names. **Absent on any pre-BC-17334 breadcrumb** — a resume that needs it and doesn't find it MUST re-run the step-1 resolution from the CSV header and write it back, never fall back to literal names (see Phase 1 step 1 § Resume fallback).
...
1. **CSV column resolution (shared mapper — BC-17334).** Resolve the CSV header onto canonical fields through the shared column mapper — do NOT string-match literal header names. This is the single column-resolution convention for the whole command (Phase 2 domain-parse and Phase 4 lead-body build read the map this step records), so launch-campaign accepts the same Apollo / Serper / Clay / hand-built header variants the `salesforce-preload` pre-load already handles.
   ...
   - **This command's required set — and where it DIVERGES from the mapper's.** The mapper bakes in the *pre-load's* requiredness (`email` + `company` required; `domain` OR `phone` as the contact method). launch-campaign owns a different set — apply it against `resolved`, and do NOT act on the mapper's own `unresolved` ambiguities (they encode the pre-load's rules, not these):
     ...
     - Divergences: `first_name` is **required here** (the mapper does not require it); `company` is **optional here** (the mapper requires it — ignore any `company`-unresolved ambiguity); the required contact field is **`domain` specifically** — a phone-only file does NOT satisfy it (Phase 2 needs an email/website domain to detect ESP, not a phone number).
   - **Operator questions — only for fields this command reads.** For each `collision` ambiguity (two headers claiming one field — e.g. `Company Website` + `Company Domain` both → `domain`) on a field in the used set, ask the operator **once** via `AskUserQuestion`, attaching three sample values per candidate column (`samples_for()` in the mapper), the same question shape the skill uses. If a **required** field (`email` / `first_name` / `domain`) is unresolved but a held-back column could be it (bare `name` lands in `unmapped` — genuinely ambiguous between a person and a business), ask the same one-shot question rather than guess. Fold every answer in via the mapper's `apply()`. **Never guess a column** — a wrong `first_name` or `domain` mis-renders every lead.
   - **HALT** if any required field is still unresolved after the operator turn — name the missing field(s) and the headers present; the operator fixes the CSV and re-runs. (This replaces the old literal-header presence check; the fail-closed behavior is unchanged, only the matching is now variant-tolerant.)
   - **Record the map.** Hold the final field→header map in scratch state; **step 10 persists it into the metadata JSON as `resolved_columns`** (the metadata file does not exist yet at this step — step 10 is the first write). Phases 2, 4, and 10 read it from scratch state during the run, and from metadata on a resume, so the operator is never re-asked. Report which optional fields resolved (absent-but-referenced-in-copy columns are still flagged separately in step 4).
   - **Resume fallback — `resolved_columns` absent.** A breadcrumb written before BC-17334 (or any run that died before step 10) has no `resolved_columns`. When a resumed phase needs the map and metadata does not carry it, **re-run this step's resolution from the CSV header** and write the result back into the metadata before proceeding — never fall back to literal column names, and never proceed on a partial map.
   - **Boundary — custom variables are NOT mapper-resolved.** The mapper covers only the six canonical lead fields above. Copy-artifact custom variables (`RECENCY_ANCHOR`, `PROOF_POINT_COMPANY`, `{COMPANY_DOMAIN}`, …) keep their existing case-insensitive column-name match in step 5 and Phase 4 step 2 — never route them through the mapper.
...
10. **Write initial metadata JSON.** First, **resolve the sending identity**: if `--identity` was provided use it; otherwise — on a real (non-`--preview`) run — prompt the operator via `AskUserQuestion` to pick `labs`, `supply`, or `nites` (no default, the operator must choose; do NOT derive it from `--entity`, identity is an independent axis). Under `--preview` (a no-mutation dry-run that never tags) skip the prompt when `--identity` is absent and record `sending_identity: null`. **Re-validate** the resolved value against the IV-11 enum regardless of source (flag or prompt) — an off-enum value HALTs, the same as an explicitly-supplied bad flag. Record it as `sending_identity` in scratch state. **Then verify the identity tag exists** in the target workspace, before any EB state is created (skipped under `--preview`, and when `sending_identity` is null): `search_api_spec` + `call_api` to list the workspace's tags and match the chosen identity's capitalized label (`labs`→`Labs`, `supply`→`Supply`, `nites`→`Nites`; the platform reconcile creates them capitalized, BC-13861). **HALT** if the chosen identity's tag is absent — a missing tag is a setup problem to fix before launching; do NOT create it here. Record `identity_tag_id` (the chosen identity's id) in scratch state for Phase 5. This is a read (no mutation — consistent with this phase) and it fails closed *before any campaign is created*, the same invariant IV-11 states for the value check. Then validate `--campaign-name` per IV-8 (regex + write-path realpath confinement) before constructing the path. Determine write path per § Launch metadata schema "Dogfood write path" note:
    ...
    Populate `schema_version`, `entity`, `sending_identity`, `campaign_name_base`, `workspace`, `copy_artifact_path`, `csv_path`, `lead_count`, `launched_at`. Also record the scratch-state values from steps 1–7: **`resolved_columns` (the step-1 column map — always written, so Phases 2/4/10 and any resume read it instead of re-resolving)**, `workspace_mismatch` (if any), `sender_resolution_method`, `unique_per_lead_enabled`. Set `last_completed_phase: 1`. This is the first progressive write.
...
## Phase 1b — SALESFORCE PRE-LOAD (personal instance only)

**Purpose.** For the **personal** Email Bison instance only, pre-load this run's cleaned lead set into Salesforce as Contacts under real business Accounts **before** the EB upload (Phase 4), so OutboundSync later matches each Contact by email instead of deriving a `gmail.com` Account that the `AccountTriggerHandler` free-email guard rejects unconditionally (BC-4776 / BC-5574). This is the run's **first mutation** — a Salesforce write — and it runs before any Email Bison state is created: if it halts, EB is untouched. It **delegates** every decision + write to the `salesforce-preload` skill (BC-17213); this phase only resolves the instance, invokes the skill, and folds its result into the run. **This is NOT suppression (BC-17224).** The pre-load asks "is this contact in Salesforce?" and answers *put it there so it can be mailed*; suppression asks the same question and answers *do not mail*. They are different rules and this phase must never become the other.

**Instance resolution (explicit, never inferred).** The target instance is read from `--workspace` — the same explicit input Phase 1 already validated; it is never guessed.

- `--workspace emailbison-personal` → **personal** → run this phase.
- `--workspace emailbison-b2b` → **commercial** → **skip**: commercial leads carry corporate emails that resolve real domains, so there is nothing to pre-load. Record `salesforce_preload: {"status": "skipped", "reason": "commercial"}` in the metadata JSON, print one line (`Phase 1b skipped — commercial instance; leads resolve real domains, nothing to pre-load`), and proceed to Phase 2.
- `--workspace` absent → **HALT** (Phase 1 already requires it; this is defense-in-depth — never default the instance).

**Idempotent resume.** Phase 1b tracks its own `salesforce_preload.status`, **not** `last_completed_phase` (which stays a 1–11 integer). On any invocation with the personal instance: if `salesforce_preload.status == "complete"`, skip and proceed to Phase 2; otherwise run the steps below — even when `last_completed_phase` already reads `1`. Do **not** treat a `last_completed_phase: 1` breadcrumb as license to skip straight to Phase 2 on the personal instance; the pre-load must be complete first. (The skill is itself idempotent — a re-run matches what exists and seeds only net-new — so re-running is safe regardless; the status check just avoids re-prompting the write-gate.)

**Preview mode (`--preview`).** Under `--preview` the whole run is a no-mutations dry-run. Run the steps below **but** invoke the skill with `--dry-run` (step 1) so it stops after its plan and **writes nothing to Salesforce**; in step 3 record `salesforce_preload: {"status": "preview", …planned counts}` instead of `"complete"`, do **not** materialize a surviving CSV, and leave `lead_count` unchanged. This keeps Phase 1b inside the command's `--preview` no-mutations contract, exactly as Phases 3–9/11 are skipped under it.

**Steps (personal instance):**

1. **Materialize the cleaned lead file, then invoke the `salesforce-preload` skill** via the `Skill` tool (pass `--dry-run` under `--preview`):
   - **First write a cleaned CSV** — the raw `--csv` **minus** the Phase-1 step-2a/2b dropped rows carried in scratch state (`duplicate` + `undeliverable`). Phase 1 keeps its cleaning in **scratch, not on disk**, and the skill takes a `--csv` **path** — so invoking it against the raw `--csv` would feed the skill the very rows Phase 1 removed, and the skill's surviving set would silently re-add them to the send. Write the cleaned CSV to a path-confined working location (IV-8 path rules apply). Because the skill only ever returns rows that were in its input, the surviving set is then a **subset** of the Phase-1 cleaned set by construction — never a superset.
   - **Inputs to the skill:** the cleaned-CSV path (above), `--instance personal`, and `--target-org marketing-claude-prod` (the Marketing Admin write identity — the same owner OutboundSync writes as; override only for a dogfood sandbox).
   - The skill runs its own flow end-to-end: map the CSV columns → read Salesforce for existing Contacts (by exact email) and Accounts (by normalized company name) via `run_soql_query` → classify each row (**net-new** / **matched** / **multiple** / **needs-review**) → render a **dry-run preview** → present **its own operator write-gate that names the personal instance out loud** → write Accounts, then Contacts, via the `sf` CLI, owned by Marketing Admin → emit a review sidecar.
   - The skill **owns** the Salesforce reads/writes, the `sf` `--target-org` guard, the seed-floor rule (a **net-new** Contact is seeded `Lifecycle_Stage__c = Cold_Prospect` / `Lead_Status__c = New`; a **matched** Contact's stage/status is never touched), and idempotency. **Do NOT re-derive any of that logic here** — this phase consumes the skill's result.

2. **Consume the skill's result.** The skill returns (a) the **surviving lead set** — every row safely represented in Salesforce (net-new created, single match attached, or already-present duplicate), with **needs-review rows removed**; (b) **counts** (accounts created, contacts created, matched, multiple flagged, held); and (c) the **review-sidecar path** — the skill's own `-preload-review.csv`, which already holds every needs-review + flagged row.
   - **Materialize the surviving set as a durable file.** Write the surviving rows to a path-confined working CSV (`{campaign-name}-{YYYY-MM-DD}-preloaded.csv`, IV-8 rules), and record its path as `salesforce_preload.surviving_csv_path` in metadata. **From Phase 2 onward, every phase that reads "the CSV" / "the lead set" reads `surviving_csv_path` when it is set — never the raw `--csv`.** This is what durably removes the held rows from the send: the raw `--csv` still contains them, and a scratch-only working set would not survive a fresh-session resume (metadata is then the only source). Set `lead_count` to the surviving count for the gate display (every downstream count check — Phase 4 chunking + step 9, Phase 6 attach — uses it), but the surviving **CSV**, not the count, is the source of truth for which rows proceed.
   - **Held (needs-review) rows never reach the EB upload** — that phase-order guarantee is the entire purpose of running before Phase 4. They are **not** re-consolidated into the launch `-skipped.csv`: they already have a complete audit trail in the skill's `-preload-review.csv`, whose path Phase 1b records in metadata (step 3). The two review artifacts stay distinct — the launch skipped-contacts file is EB-side list hygiene; the pre-load sidecar is the Salesforce-side disposition.

3. **Record + outcome.** Append to the metadata JSON: `salesforce_preload: {"status": "complete", "instance": "personal", "target_org": "<org>", "accounts_created": N, "contacts_created": N, "matched": N, "multiple_flagged": N, "held": N, "planned_vs_created": [P, C], "sidecar_path": "<path>|null", "surviving_csv_path": "<path>"}`, **and re-write the top-level `lead_count` to the surviving count from step 2** — otherwise a fresh-session resume (scratch state gone, metadata the only source) would keep the pre-reduction count. **Leave `last_completed_phase` at 1** — Phase 1b is a sub-phase of PRE-FLIGHT, so the integer breadcrumb stays monotonic (1 → 2 → …). If the skill **HALTED** (e.g. the entire list is "no company", or the operator aborted the write-gate), **record `salesforce_preload: {"status": "halted", ...}`** and **HALT the campaign** — nothing proceeds to Phase 2 or the EB upload. Recording a non-`complete` status is what lets a later resume detect the pre-load is unfinished (see the resume rule in § Inputs and § Launch metadata schema).

**If Phase 1b fails or halts:** Salesforce may hold partial writes; the skill is idempotent (a re-run matches what exists and seeds only net-new), and **no Email Bison state has changed**. Fix the input or the Salesforce-side issue and re-run — the pre-load resumes cleanly (via its `salesforce_preload.status` check), and Phases 2–11 run on the surviving set.
...
**Purpose.** Create lead records in the EB workspace with per-lead `custom_variables` values. Leads exist as workspace-level records first; Phase 6 attaches them to specific campaigns. This is the first **Email Bison**-destructive phase in the flow (Phase 1b's earlier write, on the personal instance, is to Salesforce, before any EB state) — leads in EB carry tracking history and touch quota, so creating them is not free.
...
2. **Prepare lead batches.** Read the lead CSV — `salesforce_preload.surviving_csv_path` when it is set (the personal-instance pre-load's surviving rows; see Phase 1b), else the raw `--csv`. For each row, build the lead object using EB's lead-body field names (NOT the CSV column names — Sx-6, BC-5906):

   ```json
   {
     "email": "<row[ resolved_columns.email ]>",
     "first_name": "<row[ resolved_columns.first_name ]>",
     "last_name": "<row[ resolved_columns.last_name ] — if resolved>",
     "title": "<row[ resolved_columns.title ] — if resolved>",
     "company": "<row[ resolved_columns.company ] — if resolved>",
     "custom_variables": [
       ...
       {"name": "company_domain", "value": "<row[ resolved_columns.domain ]>"}
     ]
   }
   ```

   **CSV → EB lead-body mapping** (the source column for each canonical field is the header `resolved_columns` bound it to in Phase 1 step 1 — never a literal CSV name):

   | Canonical field (`resolved_columns` key) | EB lead-body field |
   |---|---|
   | `email` | `email` |
   | `first_name` | `first_name` |
   | `last_name` | `last_name` *(only if resolved)* |
   | `title` | `title` |
   | `company` | `company` *(only if resolved)* |
   | `domain` | (no native EB field — stash as a `custom_variable` named `COMPANY_DOMAIN`, or drop if unused by copy artifact) |

   The `domain` field has no native EB lead-body field: stash it as the `COMPANY_DOMAIN` custom variable when the copy artifact references `{COMPANY_DOMAIN}`, else drop it. (Phase 2 ESP detection parses each lead's *email* domain, not this column — see Phase 2 step 2.)
...
7. **Per-chunk loop with turn-structure preservation.** For each chunk `i` in 1..M:
   ...
   d. **Workspace-collision recovery (BC-14044) — on HTTP 422 only.** A 422 means ≥1 email in this chunk already exists as a workspace lead (`/api/leads/multiple` is atomic + non-upserting — the whole chunk rejected, nothing created; Sx-8, BC-11072). `call_api` strips the body to `{error: "HTTP 422 Error"}` with no per-row detail, so identify the collisions client-side:
      i. **Build the workspace existing-email set (once per Phase 4, cached).** Paginate the target workspace's `list_leads` by incrementing `page` from 1 until every page is retrieved — each response's `Page X of N (M total leads)` header reports the total page count `N`, so continue until `X` reaches `N` (equivalently, until the collected email count equals `M`); **never stop on an early short or empty page**, because an incomplete scan misses collisions and would false-HALT a recoverable 422 at step ii. EB ignores `per_page` and hard-caps 15/page, so at ~15,400 leads this is ~1,029 GETs, incurred on every 422 incl. the re-run case. Collect each `email.strip().lower()` into a set. This reactive scan is a backstop — the opt-in proactive single-workspace exclusion pre-pass in **BC-16147** filters before upload to avoid the 422 (and this scan) entirely. A later chunk's 422 reuses the cached set — never re-page per chunk. Cost is paid ONLY on a real 422 (the rare / re-run case), never on a clean launch.
...
2. **Build the variable values map.** For each `{VARIABLE}` extracted from step_1/step_2 subject+body:
   - EB-standard variables (`FIRST_NAME`, `LAST_NAME`, `COMPANY`) resolve from the lead's CSV row through `resolved_columns` (the `first_name` / `last_name` / `company` fields bound in Phase 1 step 1) — never a literal column name, or this preview would miss the very header variants Phase 1 accepted. If the field is optional and didn't resolve, fall through to the `custom_variables[].default` rule below.
