...
7. **Post-attach verification (count-scalar first; paginate on mismatch).** The invariant enforcement step. For each campaign ID:
   ...
   - **Identity-membership assertion (not just count).** The count check above is self-referential — it only proves the attach matches step 2's enumeration. Independently assert the attached-sender set is a **subset of** step 2's verified identity-tagged list: every attached sender id must appear in it. If any attached sender falls outside it — e.g. an off-identity sender lingering on a *reused* pre-BC-13864 campaign (step 6 "Reuse existing IDs"), which step 9b's tag-only check doesn't catch — **HALT and surface** the campaign + the offending sender ids; do not advance `last_completed_phase`. (Correct whether `attach_sender_emails_to_campaign` replaces or appends the sender set.)
