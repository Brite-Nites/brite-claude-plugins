...
**Identity pre-check (resume-safe, BC-13863).** Before any campaign is created (step 6), ensure the identity tag is resolved and exists in the target workspace. A fresh run already did this in Phase 1 step 10 — but a **resumed run skips Phase 1**, and a `--preview` breadcrumb **skipped resolution** even with an explicit `--identity`. So if the identity tag ids aren't already in scratch: resolve `sending_identity` if null (prompt), then `search_api_spec` + `call_api` to list the workspace tags once, capture `identity_tag_id` + the other identity ids, and **HALT if the chosen identity's tag is absent**. This closes the missing-tag failure *before* step 6 creates campaigns on **every** path (fresh, resumed, preview-then-resume) — so a misconfigured workspace never orphans campaigns.
