...
1. **Invoke the `salesforce-preload` skill** via the `Skill` tool, on the Phase-1 **cleaned** lead set:
   - **Inputs:** the cleaned CSV (post-Phase-1 dedup + deliverability filter), `--instance personal`, and `--target-org marketing-claude-prod` (the Marketing Admin write identity — the same owner OutboundSync writes as; override only for a dogfood sandbox).
...
2. **Consume the skill's result.** The skill returns (a) the **surviving lead set** — every row safely represented in Salesforce (net-new created, single match attached, or already-present duplicate), with **needs-review rows removed**; (b) **counts** (accounts created, contacts created, matched, multiple flagged, held); and (c) the **review-sidecar path**.
   - The **surviving set replaces the cleaned lead set** that Phases 2–11 consume; set `lead_count` to the surviving count (every downstream count check — Phase 4 chunking + step 9, Phase 6 attach — then uses this reduced count, exactly as it uses the Phase-1 cleaned count today).
   - **Held (needs-review) rows** are stashed in scratch state with `skip_reason: sf_preload_held`, to be written by the **Phase 2 step 4c consolidated skipped-contacts writer** alongside the role/personal + dedup/undeliverable rows. (Under `--no-host-lookup`, Phase 2 does not run — the Phase 1 step 2d fallback writer covers them at the same path with the same IV-8/IV-9 treatment.) **Held rows never reach the EB upload** — that phase-order guarantee is the entire purpose of running before Phase 4.

3. **Record + outcome.** Append to the metadata JSON: `salesforce_preload: {"status": "complete", "instance": "personal", "target_org": "<org>", "accounts_created": N, "contacts_created": N, "matched": N, "multiple_flagged": N, "held": N, "planned_vs_created": [P, C], "sidecar_path": "<path>|null"}`. **Leave `last_completed_phase` at 1** — Phase 1b is a sub-phase of PRE-FLIGHT, so the integer breadcrumb stays monotonic (1 → 2 → …). If the skill **HALTED** (e.g. the entire list is "no company", or the operator aborted the write-gate), **HALT the campaign** — nothing proceeds to Phase 2 or the EB upload.
