...
| `persona-doc-author` | n/a | persona-doc markdown (whole file — no builder) | persona authoring (BC-12905 C2); sibling to `story-doc-author`/`journey-doc-author` |
| `fidelity-reviewer` | L3 (per-issue) | issue-fidelity verdict (STRUCTURE) | `flow-linear-scaffold` Q13.3 |
| `quality-reviewer` | n/a (per-doc) | substance verdict `{result, per_dimension, findings, cosmetic_ignored}` (PASS/CONCERNS) | WS-B substance gate, sibling to `fidelity-reviewer`; B-3 wires it into `flow-doc-author`/`flow-journey-author`; B-2 eval + WS-E adversarial review consume it; `doc_kind: persona_doc` (P1–P5) added by BC-12905 C2 |
...
- **MCP reuse (Q30.4 + Q32):** all Linear access goes through `mcp__plugin_workflows_linear-server__*`. FDA's own `.mcp.json` registers only `gbrain-team` (the Brite handbook brain, via `scripts/gbrain-team-broker.sh`); it deliberately does **not** register a Linear server — registering a duplicate Linear server breaks tool routing (per [BC-5810](https://linear.app/brite-nites/issue/BC-5810) § 4 and [BC-5811](https://linear.app/brite-nites/issue/BC-5811) § 4.2).
...
- **Clone re-sync obligation (Q51/Q52/Q53; guarded by [BC-7060](https://linear.app/brite-nites/issue/BC-7060) + [BC-12410](https://linear.app/brite-nites/issue/BC-12410)).** Each cloned command carries an `Upstream-SHA:` header recording the upstream blob at clone time. **When you edit a cloned upstream** (`plugins/workflows/commands/{session-start,review,ship}.md`), re-sync the matching FDA clone **in the same PR**: port the change (respecting the locked FDA-swap axes), bump the clone's `Upstream-SHA`, and confirm `bash plugins/flow-architecture/scripts/check-clone-drift.sh` → `pass=3 fail=0`. **The upstream-editing author owns this re-sync** — escape hatch: revert the upstream edit, or loop in the FDA owner, if the change touches a locked FDA-swap axis you cannot reconcile. Two advisory CI guards back this (neither hard-blocks; promotion to hard-block is the BC-7060 v1.1 criterion): `clone-drift-check` (BC-7060 — lagging origin/main backstop, fires on every PR) and `shift-left-clone-drift` (BC-12410 — `scripts/check-clone-drift-shiftleft.sh`, a path-filtered gate that fires **on the editing PR** so the obligation surfaces at the source, not on the next unrelated PR).
...
- **Registered by FDA** — `mcp__plugin_flow-architecture_gbrain-team__*` (Brite handbook brain via `scripts/gbrain-team-broker.sh`; the sole server in FDA's `.mcp.json`).
...
Per Q29 lock + Q29 amendment 6 (LOCKED 2026-06-26 per BC-13935), the plugin owns a **37-gate quality stack** across three categories — phase-transition gates (9, including the `journey-front-matter-populated` gate added per Q29 amendment 6), per-flow discipline-child gates (~22), and cross-cutting consistency gates (6, including the `cross-domain-deps-bidirectional` gate enforcing 1:1 mirror between story-doc `## Cross-domain dependencies` section (Q27 amendment 1 mod 4) and Linear `blockedBy` relations on sub-flow parent issues). Full enumeration lives in `docs/design-rationale/fda-plugin-architecture-overview.md` § 3h; do not duplicate the list here.

**Status-vs-code advisory (BC-12909, Q38 amendment 1 — outside the 37).** Separately from the 37 hard gates, `/flow:audit` emits one **agent-backed advisory soft-warn**: the status-vs-code cross-check dispatches the existing `codebase-inferrer` and diffs each story doc's declared `status:` against the fresh `status_inferred` in both directions (deflation = doc under-sells built code; inflation = doc over-claims), gated on a repo code root (`src/`/`app/`). Human-adjudicated, never auto-fails, never moves the exit code, and **not counted in the 37**. See `commands/audit.md` § Status-vs-code advisory.
...
- **`/flow:audit` vs `/flow:review` (Q52 sub-decision 4; cross-cutting requirement #5).** `/flow:audit` runs FDA-process-compliance gates (filesystem-existence + Linear-state checks against the 37-gate stack post-Q29 amendment 6). `/flow:review` runs code-review agents on a diff (P1/P2/P3 findings classification, simplification pass). Distinct purposes. `/flow:audit` auto-invokes before `/flow:ship`; `/flow:review` is invoked when the user wants diff-level review. A `--audit-preflight` flag for `/flow:review` is a v1.1 candidate (parking lot #48) if Brand Hub dogfood reveals demand for bundled coverage.
