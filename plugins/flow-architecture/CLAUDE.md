...
Per Q29 lock + Q29 amendment 2 (LOCKED 2026-05-26 per BC-10729), the plugin owns a **36-gate quality stack** across three categories — phase-transition gates (8), per-flow discipline-child gates (~22), and cross-cutting consistency gates (6, including the new `cross-domain-deps-bidirectional` gate enforcing 1:1 mirror between story-doc `## Cross-domain dependencies` section (Q27 amendment 1 mod 4) and Linear `blockedBy` relations on sub-flow parent issues). Full enumeration lives in `docs/design-rationale/fda-plugin-architecture-overview.md` § 3h; do not duplicate the list here.
