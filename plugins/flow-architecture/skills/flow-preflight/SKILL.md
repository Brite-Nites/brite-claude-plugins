...
**Optional field — `frontmatter_schema` (BC-12572).** A consumer repo MAY carry `"frontmatter_schema": "strict"` to widen the `/flow:audit` `story-front-matter-populated` gate from its 4-key presence floor (`flow_id`/`status`/`figma`/`user_docs_url`) to the **full 20-key story canon** (ADR-029 + the job-story template; presence, **never** non-emptiness — honest-empty `personas: []` / `qa_last_signed_off: null` pass). Same construction + lifecycle as `story_frame` above: **`flow-preflight` does NOT write it**; it is set **manually** once a repo's story docs have been migrated to full canon; absent / any non-`"strict"` value ⇒ the default **`lenient`** 4-key floor (today's behavior, so no consumer repo ship-halts on plugin update); it is a TRANSIENT strangler-fig deleted once all WS-E repos converge (kebab repos last, after BC-13152). The always-on truth surface is the standalone `scripts/flow-frontmatter-lint.sh` (A-10 story / A-11 journey), which reports the full canon + drift keys regardless of this flag — this gate is its per-repo *enforcement* counterpart.
