...
**Ownership note for `LINEAR_ISSUE_COUNT`:** the **orchestrator** (not flow-preflight) is responsible for calling `mcp__plugin_workflows_linear-server__list_issues` and passing the result in via env var before invoking flow-preflight. `list_issues` is intentionally absent from this skill's `allowed-tools` — if the env var is unset at runtime, flow-preflight treats it as 0 and degrades to `greenfield` (correct for empty projects). Orchestrator-side call: `list_issues` scoped to the project from 6.3a with **`limit: 10`** — the threshold IS the cap, so a returned count of exactly 10 means "≥ 10" without paginating. Counting the full backlog (potentially hundreds of issues across multiple round trips) is wasteful when the heuristic only needs to distinguish "< 10" from "≥ 10".
