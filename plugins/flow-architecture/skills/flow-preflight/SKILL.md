...
**OBSOLETE field — `story_frame` (Q29 amendment 3 / BC-11983 → collapsed by amendment 4 / BC-12197).** This field is **no longer read.** It was a transient strangler-fig: a consumer repo carried `"story_frame": "strict"` to narrow the `/flow:audit` `story-job-story-regex` gate so the retired constraint-spec frame (`**Given**`+`**MUST**`+`**so that**`) no longer passed — only the human job-story frame did. Once **all FDA consumer repos** reached `strict`, BC-12197 collapsed the flag: the `story-job-story-regex` gate is now **unconditionally human-frame-only** (constraint-spec rejected everywhere). A leftover `"story_frame": "strict"` key in an existing `.flow/config.json` is a harmless **ignored extra** — do NOT add it to a forbidden-keys list (the `preflight-complete` gate is a required-subset check, not a closed allowlist, so extra keys are safe but unread). New repos need not set it.

**OBSOLETE field — `frontmatter_schema` (BC-12572 → collapsed by BC-13915).** This field is **no longer read.** It was a transient strangler-fig (the structural twin of `story_frame`): a consumer repo carried `"frontmatter_schema": "strict"` to widen the `/flow:audit` `story-front-matter-populated` gate from a 4-key presence floor to the **full 20-key story canon** (ADR-029 + the job-story template; presence, **never** non-emptiness — honest-empty `personas: []` / `qa_last_signed_off: null` pass). Once **all FDA consumer repos** reached `strict`, BC-13915 collapsed the flag: `story-front-matter-populated` (and the redirect-canon + journey-frontmatter checks) now enforce the full canon **unconditionally**. A leftover `"frontmatter_schema": "strict"` key in an existing `.flow/config.json` is a harmless **ignored extra** — do NOT add it to a forbidden-keys list (the `preflight-complete` gate is a required-subset check, not a closed allowlist). New repos need not set it. The always-on truth surface is the standalone `scripts/flow-frontmatter-lint.sh` (A-10 story / A-11 journey), which reports the full canon + drift keys; this gate is now its unconditional *enforcement* counterpart.
