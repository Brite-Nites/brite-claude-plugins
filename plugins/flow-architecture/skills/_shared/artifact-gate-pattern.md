...
- [Story] — 5 checks (file exists, front-matter populated — unconditionally the full 20-key story canon (ADR-029 + job-story template), presence not non-emptiness (BC-13915 collapsed the per-repo `frontmatter_schema` strangler-fig to this hardcoded full-canon end-state); the standalone `flow-frontmatter-lint.sh` A-10/A-11 is the always-on truth surface that also names drift keys — story-frame regex match — the canonical human job-story `When/I want to/so I can` (operator- or customer-anchored even for infra, per rubric D11) is the ONLY accepted frame; the legacy constraint-spec `Given/MUST/so that` is rejected (BC-12197 collapsed the per-repo strangler-fig to this hardcoded human-only end-state), 3-5 Gherkin scenarios, `verify-docs.sh` passes for the doc).
