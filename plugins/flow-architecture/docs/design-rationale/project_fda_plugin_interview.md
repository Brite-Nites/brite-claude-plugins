...
|---|---|---|---|
| Domain slug case | UPPERCASE kebab-case (implied via examples) | **lowercase kebab-case** (`^[a-z][a-z0-9-]*$`) | `flow-classify-domain-state.sh:46` |
| H3 backtick wrap | bare only (`### <DOMAIN> ...`) | **either backtick-wrapped or bare** (`^### \`?<DOMAIN>\`?[[:space:]]`) | `flow-classify-domain-state.sh:70` |
