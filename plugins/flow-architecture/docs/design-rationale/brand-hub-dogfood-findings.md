...
| _pending file_ | **P0** | **2 (Step 5 L1 dispatch — BLOCKER)** | **All 4 L1 reviewer agents (`plan-ceo-reviewer`, `plan-design-reviewer`, `plan-eng-reviewer`, `plan-devex-reviewer`) are NOT registered as dispatchable subagent types.** Available agent list in this session: 28 agents covering `cadence:*` / `workflows:*` / `vercel:*` / core — ZERO `flow-architecture:*` or `flow:*` or `plan-*` entries. Plugin IS registered in `.claude-plugin/marketplace.json` at v0.2.22; agent files DO exist on disk with `name:` frontmatter (`name: plan-ceo-reviewer`). **Blast radius**: Phase 2 office-hours L1 (4 reviewers), Phase 4 L2 (CEO + Design per-domain), Phase 5 L3 (5 disciplines per-sub-flow + fidelity-reviewer + codebase-inferrer + inventory-author), Phase 6 story-doc-author, Phase 7 journey-doc-author, L4 plan-X reviewers. **The entire FDA multi-perspective review + authoring agent surface is non-functional from a fresh session dispatch.** Diagnosis converged 2026-05-12: `plugin.json` shape (10 keys including `commands: "./commands/"` + `skills: "./skills/"`) is byte-identical to workflows + cadence, ruling out plugin-schema issue. Custom `mode: four-mode` frontmatter on 7 of 12 agents was a suspect — but the 5 agents WITHOUT `mode:` (story-doc-author, journey-doc-author, inventory-author, codebase-inferrer, fidelity-reviewer) ALSO don't appear, so failure is plugin-wide not per-agent. Hooks-dir absence isn't the cause (cadence + workflows-server + others register without SessionStart hooks). Most-likely cause: **agent registration ran at session start before the worktree's plugin source was discoverable, OR /reload-plugins is needed after recent plugin version bumps to refresh agent enumeration.** **`/reload-plugins` retest (2026-05-12, same session)**: ran twice. Output: `Reloaded: 6 plugins · 39 skills · 28 agents`. Agent count unchanged (still 28; flow-architecture contributes 0). Re-tested Agent dispatch with 3 namespace variants — `plan-ceo-reviewer`, `flow-architecture:plan-ceo-reviewer`, `flow:plan-ceo-reviewer` — all 3 returned identical "Agent type ... not found" with same 28-agent available list. Independent confirmation that the bug is structural, NOT a registration-refresh issue. **Plus same-turn skill list shows zero `flow-architecture:*` or `flow:*` entries among the 39 registered skills** — despite 11 `SKILL.md` files on disk under `plugins/flow-architecture/skills/`. **So flow-architecture loads as 1 of the 6 plugins (probably contributes its 15 slash commands, since orchestrator markdown is readable + the user's session-start operated through plugin slash-command surface area) but contributes ZERO agents AND ZERO skills.** The discovery bug is plugin-wide and `agents/`-+-`skills/`-specific. Updated fix-recipe hypothesis: not a refresh issue. Likely candidates: (a) a single malformed agent or SKILL.md frontmatter that crashes the loader and silently aborts directory enumeration for both `agents/` AND `skills/` (workflows + cadence don't hit this because their files don't carry custom fields); (b) a structural issue specific to flow-architecture's directory layout (e.g., a non-SKILL.md file inside `skills/` directories that confuses the loader); (c) a YAML parse error in one of the 7 reviewers' `mode: four-mode` interactions with another field. Iter-2 fix sequence: (i) systematic frontmatter audit of every `agents/*.md` + `skills/*/SKILL.md` file in flow-architecture, normalizing to workflows/cadence frontmatter shape (remove `mode: four-mode` first as the lowest-hanging suspect); (ii) re-run `/reload-plugins`; (iii) confirm agent+skill counts increase; (iv) re-run `/flow:retrofit-project` against Brand Hub. Defense in depth same as above: `mode: four-mode` should leave agent frontmatter regardless. |
