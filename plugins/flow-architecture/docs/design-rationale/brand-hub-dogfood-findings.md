...
| _pending file_ | **P0** | **2 (Step 5 L1 dispatch — BLOCKER)** | **All 4 L1 reviewer agents (`plan-ceo-reviewer`, `plan-design-reviewer`, `plan-eng-reviewer`, `plan-devex-reviewer`) are NOT registered as dispatchable subagent types.** Available agent list in this session: 28 agents covering `cadence:*` / `workflows:*` / `vercel:*` / core — ZERO `flow:*` or `plan-*` entries. Plugin IS registered in `.claude-plugin/marketplace.json` at v0.2.22; agent files DO exist on disk with correct `name:` frontmatter (`name: plan-ceo-reviewer`). Yet the Agent tool rejects them. **Blast radius**: not just Phase 2 office-hours — also Phase 4 L2 reviewers (CEO + Design per-domain), Phase 5 L3 reviewers (5 disciplines per-sub-flow), Phase 6 story-doc-author, Phase 7 journey-doc-author, Phase 5 fidelity-reviewer, and the L4 plan-X discipline reviewers. **The entire FDA multi-perspective review system is non-functional from a fresh session dispatch.** Cf. cadence/vercel/workflows agents which DO register under their plugin-name-prefix namespaces. Possible causes: (a) agent frontmatter missing a required field cadence/workflows have (compare `cadence/agents/*.md` frontmatter to `flow-architecture/agents/*.md` — both look identical at first read; need a deeper schema diff); (b) plugin discovery for flow-architecture is incomplete (the plugin's commands appear callable but agents don't); (c) `/reload-plugins` may be needed after recent plugin version bumps. **Fix priority: this is the BC-6998 critical blocker.** Before any iter 2 of the dogfood, this must be resolved. |
