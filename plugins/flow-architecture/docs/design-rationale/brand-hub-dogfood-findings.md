...
| `crm-sync` | _deferred_ | _deferred_ | ⏸ DEFERRED | _N/A_ | _N/A_ | _none_ | Deferred per BC-10321 § Out-of-scope text: CRM-target decision (HubSpot vs Salesforce) has not landed; child BC-9564 stays Backlog. Decision surfaced during iter-3 batch 1 setup: prior executor session-close report inferred "SF-only confirmed" but no explicit user confirmation exists. Conservative interpretation: honor BC-10321 as-written until decision is verified. |
| `analytics-dashboard` | _batch 2_ | _batch 2_ | _pending_ | _N/A_ | _N/A_ | _pending_ | Slated for iter-3 batch 2 (separate session). |
| `access-governance` | _batch 3_ | _batch 3_ | _pending_ | _N/A_ | _N/A_ | _pending_ | Slated for iter-3 batch 3. |
| `data-quality-migration` | _batch 3_ | _batch 3_ | _pending_ | _N/A_ | _N/A_ | _pending_ | Slated for iter-3 batch 3. |
| `ops-hardening` | _batch 3_ | _batch 3_ | _pending_ | _N/A_ | _N/A_ | _pending_ | Largest of remaining domains (8 sub-flows); slated for iter-3 batch 3. |
