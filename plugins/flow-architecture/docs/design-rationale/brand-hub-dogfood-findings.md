...
| _pending file_ | **P0** | **2 (Step 5 L1 dispatch — BLOCKER)** | **All 4 L1 reviewer agents (`plan-ceo-reviewer`, `plan-design-reviewer`, `plan-eng-reviewer`, `plan-devex-reviewer`) are NOT registered as dispatchable subagent types.** Available agent list in this session: 28 agents covering `cadence:*` / `workflows:*` / `vercel:*` / core — ZERO `flow-architecture:*` or `flow:*` or `plan-*` entries. Plugin IS registered in `.claude-plugin/marketplace.json` at v0.2.22; agent files DO exist on disk with `name:` frontmatter (`name: plan-ceo-reviewer`). **Blast radius**: Phase 2 office-hours L1 (4 reviewers), Phase 4 L2 (CEO + Design per-domain), Phase 5 L3 (5 disciplines per-sub-flow + fidelity-reviewer + codebase-inferrer + inventory-author), Phase 6 story-doc-author, Phase 7 journey-doc-author, L4 plan-X reviewers. **The entire FDA multi-perspective review + authoring agent surface is non-functional from a fresh session dispatch.** Diagnosis converged 2026-05-12: `plugin.json` shape (10 keys including `commands: "./commands/"` + `skills: "./skills/"`) is byte-identical to workflows + cadence, ruling out plugin-schema issue. Custom `mode: four-mode` frontmatter on 7 of 12 agents was a suspect — but the 5 agents WITHOUT `mode:` (story-doc-author, journey-doc-author, inventory-author, codebase-inferrer, fidelity-reviewer) ALSO don't appear, so failure is plugin-wide not per-agent. Hooks-dir absence isn't the cause (cadence + workflows-server + others register without SessionStart hooks). Most-likely cause: **agent registration ran at session start before the worktree's plugin source was discoverable, OR /reload-plugins is needed after recent plugin version bumps to refresh agent enumeration.** **`/reload-plugins` retest (2026-05-12, same session)**: ran twice. Output: `Reloaded: 6 plugins · 39 skills · 28 agents`. Agent count unchanged (still 28; flow-architecture contributes 0). Re-tested Agent dispatch with 3 namespace variants — `plan-ceo-reviewer`, `flow-architecture:plan-ceo-reviewer`, `flow:plan-ceo-reviewer` — all 3 returned identical "Agent type ... not found". Independent confirmation: bug is structural, not a registration-refresh issue. **Important refinement (caught 2026-05-12 same session): the 10 flow-architecture sub-skills all declare `disable-model-invocation: true` per Q7 lock — they ARE registered (counted in the 39-skill total) but intentionally hidden from the model's skill-list per Q7 design. SKILLS ARE NOT BROKEN. Only the agents are broken.** Scope narrows from "plugin-wide agents + skills" to "agents/ directory specifically." **Narrowed hypothesis (high-confidence): the custom `mode: four-mode` field on 7 reviewer agents (`plan-{ceo,design,eng,devex,story,qa,docs}-reviewer.md`) triggers silent rejection by Claude Code's agent loader, which then aborts enumeration for the entire `agents/` directory — taking the 5 non-`mode` agents (`codebase-inferrer`, `fidelity-reviewer`, `inventory-author`, `journey-doc-author`, `story-doc-author`) down with them.** Evidence: (1) workflows + cadence agents register and use only the documented allowlist (`name`, `description`, `model`, `tools`, plus optional `license`, `allowed-tools`, `metadata`, `version`, `user-invocable`, `disable-model-invocation`); (2) flow-arch is the ONLY plugin with a custom `mode:` field on agent frontmatter; (3) /reload-plugins is a no-op so the bug is content-level not cache-level; (4) the YAML parse-check is clean (no tabs, frontmatter delimiters intact), suggesting the issue is at the schema-validation layer not the YAML-parse layer (similar to CLAUDE.md's documented plugin.json strict-schema gotcha — "Any unrecognized field causes silent hard failure with no error").

**Iter-2 fix sequence (estimated ≤30 min):**

1. Strip `mode: four-mode` from the 7 reviewer agent frontmatter blocks (a single-line removal per agent). The semantic value of "this agent returns four-mode output" is already documented in each agent's `description` field and in `skills/_shared/four-mode-framework.md` — keeping it in description is sufficient.
2. Bump plugin version `0.2.22 → 0.2.23` in BOTH `plugins/flow-architecture/.claude-plugin/plugin.json` + the matching `.claude-plugin/marketplace.json` entry per the BC-6000 same-commit rule.
3. Commit + push the plugin fix.
4. From a fresh session, `/reload-plugins` then confirm `flow-architecture:plan-ceo-reviewer` (or whatever namespace resolves — likely bare `plan-ceo-reviewer` or `flow-architecture:*` based on convention) appears in the Agent tool's available list.
5. Re-invoke `/flow:retrofit-project` against Brand Hub. Phase 1 resumes from the breadcrumb at `current_phase: 2` (intact on disk); Phase 2 office-hours can lift the captured 6-section intent draft from the dogfood-findings doc rather than re-running the interview (or save it to `breadcrumb.office_hours_state.section_answers` before resume so the orchestrator skips re-prompting). |
