...
**Q29 amendment 4 — collapse the transient `story_frame` flag; hardcode the human-only frame (LOCKED 2026-06-25 per BC-12197, end-state of amendment 3).** Amendment 3 introduced `story_frame: strict` as a *transient strangler-fig* with a pre-blessed three-step lifecycle whose step 3 was "once all FDA consumer repos are `strict`, REMOVE the flag and hardcode `strict` as the global end-state." That trigger was met on 2026-06-25 — this amendment is step 3.

- **Trigger met (real-data census, not the stale checklist).** Amendment 3's step-3 trigger named a 2026-06-02 WS-E repo list (`brite-sites`, `brite-roster`, `brand-hub`, `brite-labs-site`, `brite-supply-react`, `brite-pim`, `brite-lseo`). That list was **stale**: `brite-supply-react` was deleted/renamed to `brite-supply-commerce` (BC-13710), and `brite-pim` is not an FDA consumer (no `.github/workflows/fda-audit.yml`, no `.flow/config.json`). The **actual live FDA consumer fleet** — every repo whose CI runs the deterministic runner on the `@fda-audit-v1` tag — is the 7 repos `brand-hub`, `brite-base`, `brite-labs`, `brite-roster`, `brite-sites`, `brite-supply-commerce`, `lseo-tool` (org code-search `filename:fda-audit.yml org:Brite-Nites path:.github/workflows fda-audit-v1`). All 7 carried `story_frame: strict` as of 2026-06-25 (BC-13807/13859/13867/13868/13869) — so hardcoding strict is a behavioral **no-op** for every current consumer.
- **Semantics.** The `story-job-story-regex` gate is now **unconditionally human-frame-only**: it accepts ONLY the human job-story frame (`**When**` + `**I want to**` + `**so I can**`) and rejects the retired constraint-spec frame (`**Given**` + `**MUST**` + `**so that**`) everywhere. There is no per-repo mode and no lenient floor — a constraint-spec-only doc FAILs in any repo. A leftover `story_frame: strict` key in an existing `.flow/config.json` is a harmless **ignored extra** (the `preflight-complete` gate is a required-subset check, not a closed allowlist), deliberately **not** added to any forbidden-keys list (that would hard-fail all 7 live configs). The 7 consumer configs were therefore left untouched (no cosmetic sweep PRs).
- **Gate ID retained; gate COUNT unchanged.** As with amendment 3, this changes the existing `story-job-story-regex` gate's behavior, not the gate stack — the canonical total stays **36 distinct gate types**.
- **Removal scope (the actual code).** The `<mode>` param + `story_frame_mode` reader + `if [ "$mode" != "strict" ]` guard were deleted from `tests/run-audit-smoke.sh` `story_frame_present`; the parallel `_config_story_frame_mode` reader + `mode` param + `if mode != "strict":` branch were deleted from `scripts/build_audit_report.py` `_story_frame_present`; the `frame_mode` thread was removed from `scripts/run_fda_ci_audit.py` (BC-13148 two-impl lockstep preserved — both predicates collapsed identically). Mode-parametrized assertions in `scripts/test_build_audit_report.sh` + `tests/run-audit-smoke.sh` collapsed to unconditional (human-PASS / constraint-FAIL / frameless-FAIL); the marker-mechanics regression guards (BC-13751 keyword-in-bold-span, word-boundary, brite-labs cross-span GAP) were preserved as unconditional negatives. Prose updated in `commands/audit.md`, `scripts/lib/flow_doc_lint.sh` (scope comment), `skills/_shared/artifact-gate-pattern.md`, and `skills/flow-preflight/SKILL.md` (field marked obsolete).
- **`frontmatter_schema` is OUT of scope.** The independent `frontmatter_schema` strangler-fig (BC-12572) is a structural twin in the same files, also 7/7 `strict` as of 2026-06-25 and therefore also collapsible — but its collapse is tracked separately (it was not part of BC-12197's scope). The runner reads `frontmatter_schema` from the same config block; this amendment removed `story_frame` handling without touching that gate.
- **Distribution.** Because the deterministic CI runner changed, this ships via a `@fda-audit-v1` tag move (the runner is distributed to all 7 consumers via that scoped moving tag, ADR-038) **plus** a plugin version bump — gated on a fleet re-check of the new runner against all 7 consumers' `origin/main` (a predicate-logic change could break the human-frame check for all 7 at once, so the re-check precedes the tag move).

Schema-evolution discipline reinforced: Q29 amendment 4 follows amendments 1+2+3 + the broader amendment-precedent canon (explicit amendment-number + audit trail; amendment 3's body preserved verbatim above as the historical lifecycle record). **Label re-drift (2026-06-25):** with amendment 4 now claimed by this collapse, the parking-lot #51 plan-X-section gate, if/when it lands, becomes **Q29 amendment 5** territory (superseding the 2026-05-31 note that reserved amendment 4 for it).

---

**Q29 amendment 5 — collapse the transient `frontmatter_schema` flag; hardcode full canon (LOCKED 2026-06-25 per BC-13915, twin of amendment 4).** `frontmatter_schema` (introduced by BC-12572 to widen the `story-front-matter-populated` gate from a 4-key presence floor to the full ADR-029 canon) was the **structural twin** of the `story_frame` strangler-fig — same per-repo `.flow/config.json` mode, same fail-safe-lenient resolution, same pre-blessed "collapse once all consumers converge" lifecycle. Amendment 4's own note flagged it as "OUT of scope... collapsible... tracked separately"; this amendment is that separate collapse, claimed the day after the trigger was met.

- **Trigger met.** All 7 live FDA consumers (`brand-hub`, `brite-base`, `brite-labs`, `brite-roster`, `brite-sites`, `brite-supply-commerce`, `lseo-tool`) carried `frontmatter_schema: strict` as of 2026-06-25 (the BC-12572 consumer-flip children BC-13291/13309/13490/13573/13672/13691/13710). Hardcoding full canon is a behavioral **no-op** for every current consumer.
- **Semantics.** Three gates that previously mode-gated on `frontmatter_schema: strict` are now **unconditional**: `story-front-matter-populated` (full 20-key story canon — presence, never non-emptiness), `redirect-front-matter-valid` (REDIRECT_CANON), and the journey-frontmatter lint (ADR-033 canon). The lenient 4-key floor was removed. A leftover `frontmatter_schema: strict` key in a consumer `.flow/config.json` is a harmless **ignored extra** (deliberately NOT added to any forbidden-keys list — that would hard-fail all 7 live configs); the 7 configs were left untouched.
- **Gate ID / COUNT retained.** As with amendment 4, this changes existing gates' behavior, not the stack — the canonical total stays **36 distinct gate types**.
- **Removal scope.** Deleted `_config_frontmatter_schema_mode` + the `mode` param on `_story_frontmatter_populated` (`build_audit_report.py`); the `fm_mode` thread + the three `if fm_mode == "strict"` branches (`run_fda_ci_audit.py`); the `frontmatter_schema_mode` reader + `<mode>` param + lenient floor (`tests/run-audit-smoke.sh`), in BC-13148 two-impl lockstep. **Fixture surgery (the material difference from amendment 4):** the canonical `audit-clean-shape` + `audit-broken-shape` fixtures shipped *lean* docs (built to exercise the now-deleted lenient floor), so all 11 fixture story docs + 4 journey docs were upgraded to full canon. The broken fixture's intentional `children.engineering` omission is a STORY_CANON key, so it now trips **two** gates (`eng-children-engineering-populated` + `story-front-matter-populated`) — the broken-fixture oracle moved from 3 → 4 hard-fails (README + both test harnesses updated in lockstep).
- **Distribution.** Same as amendment 4 — `@fda-audit-v1` tag move (the runner changed) + plugin version bump, gated on a fleet re-check of the new runner against all 7 consumers' `origin/main`.

Schema-evolution discipline reinforced: Q29 amendment 5 follows amendments 1-4 + the broader amendment-precedent canon. **Label re-drift (2026-06-25):** with amendment 5 now claimed by this collapse, the parking-lot #51 plan-X-section gate, if/when it lands, becomes **Q29 amendment 6** territory.

---

**Q29 amendment 6 — add 9th phase-transition gate `journey-front-matter-populated` (LOCKED 2026-06-26 per BC-13935, CI↔local-audit parity).** The deterministic CI runner (`run_fda_ci_audit.py`) has linted journey-doc frontmatter against the ADR-033 canon since BC-13819 (via `_journey_docs` + `flow_frontmatter_lint`). But `/flow:audit`'s Phase-B evaluator (`build_audit_report.evaluate()`) only emitted `journey-complete` — a per-domain file-*existence* check — and never linted journey *frontmatter*. So a journey missing an ADR-033 key (e.g. `display_name`, `linear_milestone`) hard-failed CI but was invisible to an author running `/flow:audit` locally: red CI they could not reproduce. Greptile surfaced this asymmetry (P1) during BC-13915. This amendment closes it by bringing the local evaluator UP to parity — option 1 (single source of truth), not weakening CI (option 2 rejected — it would lose BC-13819's coverage).

- `journey-front-matter-populated` (journey-authoring phase) — every journey doc's frontmatter satisfies the ADR-033 9-key canon (`flow_frontmatter_lint.lint_*(…, "journey")`: no MISSING canon key, no DRIFT key). It is the frontmatter sibling of `journey-complete` (which checks existence only), one phase concern deeper. Hard gate; override via Q29.5 standard flow.
- **All-journeys model (lockstep, not per-domain).** The gate iterates the **shared `_journey_docs(repo)` set** — every `journeys/*.md` minus `flow_index: skip` overview/index docs — NOT just the per-domain `journeys/{domain}.md` that `journey-complete` checks. This is deliberate: the CI runner's `_journey_docs` is broader than the per-domain model (it catches orphan/overview journeys), so a per-domain evaluator gate would re-introduce the very asymmetry being closed (an orphan journey missing a key would pass local audit while failing CI). `_journey_docs` was **moved into `build_audit_report.py`** (sibling of `_story_docs`) and the runner now imports it as `bar._journey_docs` — one source consumed by both impls per the BC-13148 two-impl-lockstep discipline.
- **Scope encodes domain membership (Greptile #497 review-fix).** A per-domain journey (`journeys/<D>.md`, `<D>` a real `flows/` domain) emits scope **`domain:<D>`** — identical to `journey-complete` for the same file — so a `--domain=<D>` audit (and the `/flow:ship --domain=<D>` that calls it) filters it **mechanically by scope-prefix** and is never blocked by an unrelated domain's or an orphan journey. A genuine orphan/overview journey whose stem matches no domain emits scope **`journey:<stem>`**, a project-level row only the unfiltered audit + CI surface. Coverage stays all-journeys (the unfiltered audit + every CI run); only the *filtered* local view is domain-scoped. The scope is computed in code (the deterministic evaluator + its bash twin), not left to LLM interpretation — that was Greptile's "enforced in code, not just documented" ask.
- **Naming asymmetry (accepted, by scope).** The CI runner emits journey frontmatter failures under its coarse runner-only umbrella id `frontmatter-schema` (scope-prefixed `journey:`), which is NOT part of the canonical manifest; the evaluator + bash twin emit the granular canonical id `journey-front-matter-populated`. Behavior is identical (same pass/fail on the same input — BC-13148 lockstep is behavioral, not string-identical); the gate-id label differs between surfaces (and, for a per-domain journey, so does the scope — the evaluator uses `domain:<D>` for `--domain` filterability while the unfiltered runner keeps `journey:<D>`), mitigated by the identical missing-key detail and the direct `domain:<D>` ↔ `journeys/<D>.md` ↔ runner `journey:<D>` mapping. Harmonizing the runner's gate-id vocabulary to the granular manifest is a separate, out-of-scope cleanup (filed as a follow-up).

**Correction to total gate count.** Post-Q29 amendment 2, the canonical total was **8 phase-transition + ~22 per-flow discipline-child + 6 cross-cutting = 36 distinct gate types**. Amendment 6 adds one phase-transition gate type — the new canonical total is **9 phase-transition + ~22 per-flow discipline-child + 6 cross-cutting = 37 distinct gate types** (multiplied by N flows for per-flow gates; phase-transition gates are counted flat by type — `journey-front-matter-populated` fires per-journey-file just as `journey-complete` fires per-domain, but each counts as one type). The post-amendment-2 "= 36" text is preserved verbatim in the prior amendment blocks per the schema-discipline amendment pattern (cf. Q29 amendment 2's "5 cross-cutting = 35" / "6 cross-cutting = 36" correction shape). Derivatives carrying the count are updated inline: `commands/audit.md` (description + body + `--gate=<id>` table + phase-transition prose), `skills/_shared/artifact-gate-pattern.md` (phase-transition section + arithmetic), `CLAUDE.md` § Quality gate stack reference, `docs/design-rationale/fda-plugin-architecture-overview.md` (TOC + §3h diagram).

**Fixture coverage.** The gate is exercised in BC-13148 two-impl lockstep *without touching the shared broken-fixture oracle*: `scripts/test_build_audit_report.sh` adds two `with_mutation` flips off the clean fixture (a dropped ADR-033 canon key → MISSING fail; an injected drift key → DRIFT fail) + a `VALID_GATE_IDS` membership assertion; `tests/run-audit-smoke.sh` adds a dedicated `2-journey` section (full-canon PASS + missing-key FAIL + drift-key FAIL, single-sourced through `lint_doc(…, "journey")` so the bash twin fails on DRIFT, not just presence). Both fixtures' journeys are full canon (BC-13915 upgraded them), so the new gate **PASSES on both** — the `audit-clean-shape`/`audit-broken-shape` oracle is unchanged (broken stays at exactly **4** hard-fails; the ADR-028 behavioral-eval golden `tests/eval/audit.golden.json` was regenerated to capture the new PASS rows, and `scripts/eval/run_eval.py`'s `ORACLE_BROKEN_FAILS` frozenset is untouched). The new gate ID `journey-front-matter-populated` is added to `run-audit-smoke.sh` § RECOGNIZED_GATES so it doesn't trip the UNCATEGORIZED-GATE-FAIL bucket.

**Distribution.** Because the deterministic CI runner (`run_fda_ci_audit.py`) changed — its local `_journey_docs` moved into `bar` — this ships via a `@fda-audit-v1` tag move (ADR-038 scoped moving tag distributing the runner to all 7 consumers) **plus** a plugin version bump (1.2.20 → 1.2.21), gated on a fleet re-check of the new runner against all 7 consumers' `origin/main`. Unlike amendments 4/5, the runner's CI behavior is **byte-identical** here (proven on the two fixtures) — the tag move is a pure refactor-distribution, not a behavioral change.

Schema-evolution discipline reinforced: Q29 amendment 6 follows amendments 1-5 + Q47 amendment 1's count-correction shape + the broader amendment-precedent canon. This is the **first count-changing Q29 amendment since amendment 2** (amendments 3/4/5 left the count at 36). **Label re-drift (2026-06-26):** with amendment 6 now claimed by this journey-frontmatter gate, the parking-lot #51 plan-X-section gate, if/when it lands, becomes **Q29 amendment 7** territory (superseding the 2026-06-25 note that reserved amendment 6 for it).
...
**Q38 sub-decision 4 deferred-decision resolution (LOCKED 2026-05-07 per Q46 lock; user-confirmed):** Stays strictly local in v1. /flow:audit emits stdout markdown + `--json` only; no Linear writeback in v1. `audit-concerns` marker type registered in Q46's `_shared/linear-writeback-pattern.md` enum but UNUSED in v1; reserved for v1.1 `--linear-surface[=parent|milestone]` flag promotion (Q38 amendment territory). Most reversible architectural choice — preserves Q38's "stdout-only by default" framing exactly; avoids notification spam from auto-invocations (auto-fires from /flow:ship + /flow:plan-{discipline} per Q38 sub-decision 5; routing those to Linear would generate ~5+ comments per ship cycle per sub-flow). /flow:ship already routes ship-summary as the team-facing checkpoint; audit-concerns is developer-internal pre-flight. **No Q38 sub-decision 4 amendment needed in v1** — the deferred decision resolved as "stay local"; v1.1 will require a Q38 amendment to add the `--linear-surface` flag if dogfood reveals demand (re-numbered to amendment 2 below now that amendment 1 has landed).

**Q38 amendment 1 — status-vs-code advisory (LOCKED 2026-06-29 per BC-12909, C3 of the BC-12905 PRD; cross-cites Q29).** `/flow:audit` gains one **agent-backed advisory** output pass. At audit time it dispatches the existing `codebase-inferrer` (BC-12909 US-15 — reuse the engine, not a new one) over the in-scope flows and diffs the returned `status_inferred` against each story doc's declared `status:`, surfacing drift in **both** directions — deflation (`declared < inferred`, the costly under-sell that hid roster's built+tested `ACL-02/03/04` + `SFI-01` behind a `NOT_STARTED` stamp) and inflation (`declared > inferred`) — as an **advisory soft-warn** in a dedicated `## Status-vs-code advisory` report section (plus a `status-vs-code-agreement` `--json` gate counted in `summary.soft_warn`). Three properties keep it honest: (1) **advisory, never auto-fail** (BC-12905 US-13) — `codebase-inferrer` is an LLM agent, so its output is not byte-deterministic; a hard gate on a coarse proxy would cry wolf and train an override reflex; (2) **outside the 37-gate hard stack** — it is NOT a Q29 gate, NOT a `--gate=<id>` value, never moves the exit code, never touches `overrides[]` — exactly as the CI-runner resolution gates (`link-resolution`, `persona-exists`) sit outside the per-flow Phase-B evaluator, so the canonical 37 count is **unchanged**; (3) **repo code-root precondition** — it runs only when `src/` or `app/` exists, else the whole pass is skipped (`no code tree to diff`) with zero warns, guarding doc-only/greenfield repos from mass false-inflation. Paired root-cause fix (US-14): `agents/codebase-inferrer.md`'s "when unsure, guess low" convention is reframed from a trusted final stamp into a **provisional scan bias reconciled at this cross-check** — the phantom "next manual gate" it cited never existed, and the shy first guess was hardening into the deflated final `status:` unchecked (`status_inferred` seeds the declared stamp at Q15.7 authoring time via `story-doc-author --status`). Composes with — does not duplicate — BC-12573 persona/journey existence + BC-12572 frontmatter + BC-12692 inventory↔doc gates (those compare doc↔doc / doc↔inventory; this compares doc↔`src/`). Tested by `tests/run-status-vs-code-vslice.sh` over the `synthetic-status-vs-code-drift` fixture (deflation / inflation / agreeing-clean / doc-only-skip) — a prose+fixture-shape v-slice (it executes no inference, mirroring `run-built-criterion-fixture-vslice.sh`), so the deterministic Phase-B oracle (`build_audit_report.py` / `run-audit-smoke.sh` / `audit.golden.json`) is untouched. Schema-discipline: this is the first LANDED Q38 amendment; the previously-reserved `--linear-surface` (parking lot #36) and `--audit-preflight` (parking lot #48) v1.1 candidates re-drift to **Q38 amendment 2** territory (whichever lands first).
...
36. **`--linear-surface` flag for /flow:audit (Q38 amendment 2 territory — re-drifted from amendment 1, now claimed by the BC-12909 status-vs-code advisory; v1.1 candidate per Q46 sub-decision 7 + Q38 deferred-decision resolution 2026-05-07).** v1: /flow:audit stays strictly local (stdout + `--json` only); no Linear writeback. v1.1: add `--linear-surface[=parent|milestone]` flag (requires a Q38 amendment) routing audit findings through Q46 as `audit-concerns` body-section or comment on chosen surface. `audit-concerns` marker type already registered in Q46's `_shared/linear-writeback-pattern.md` enum but unused in v1 — reserved for this promotion without further Q46 amendment needed. Promote if Brand Hub dogfood reveals team-visibility demand for audit findings.
...
1. **Seed the doc templates (Q58 manifest 9 → 11).** The plugin now ships `templates/docs/templates/{domain-journey,job-story}.md` (canonical structure + the FDA-additive sections — `## L2 review summary` per Q26 mod 2, conditional `## Cross-domain dependencies` per Q27 amendment 1 mod 4, `## Status notes`, and an evidence-anchor `## Status` section [grill D4]). Both `/flow:start-project` + `/flow:retrofit-project` Phase 1 templates-scaffold copy them into the consumer's `docs/templates/`. No template tokens are sed-substituted (the journey template's `linear_project_id: <LINEAR_PROJECT_ID>` line — formerly the only substituted token — was dropped per ADR-033); all authoring placeholders (`<DOMAIN>`, `<DOMAIN-NN>`, `<role>`) pass through intact.
...
**Sub-decision 5 — Scaffold wiring: create-when-absent, inject-when-present, never clobber.** `/flow:start-project` and `/flow:retrofit-project` Phase 1 templates-scaffold gain the helper in the copy manifest (12 → 13 files) plus a best-effort, **non-aborting** hook-wiring step: if `scripts/pre-commit.sh` exists, inject one idempotent `bash scripts/precommit-flow-index.sh || true` line; if it does not, create the consumer's first `scripts/pre-commit.sh` sourcing the helper; then add a `prepare` npm script (fleet-command precedent) that installs the tracked hook into `.git/hooks/pre-commit` on `npm install`, added only when no `prepare` already exists. Wiring failure never aborts a scaffold (consistent with the fail-open framing). **Amendment (v1.2.35, per a Greptile review of the backfill PRs):** the `prepare` installer is itself **non-clobbering** — it installs into `.git/hooks/pre-commit` only when that path is absent or already the managed hook (`grep -q precommit-flow-index`), leaving a developer's own local hook untouched. The install line stays `|| true`-guarded so a `cp`/`chmod` failure never propagates a non-zero exit to `npm install` (fail-open preserved end-to-end). This carries the never-clobber principle into the install step (the original fleet-precedent `cp` was unconditional). It also resolves the hook path via `git rev-parse --git-path hooks/pre-commit` rather than a hard-coded `.git/hooks/`, so the installer works from a **git worktree** (where `.git` is a file, not a directory) instead of silently skipping. (fleet-command's own reconciliation additionally broadens the managed-hook sentinel to recognize its PR-#18 `regenerate-flow-index` hook as upgradeable — a repo-local detail, not the canonical.)
