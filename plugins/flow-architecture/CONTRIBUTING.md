...
All FDA plugin work tracks under milestone **Flow-Driven Architecture Plugin v1.0** in the Brite Skill Packs project (Layer C; renamed from "Brite Skill Packs" 2026-05-27). Issues carry the `flow-architecture` label plus a categorical (`skill` / `agent` / `command` / `infrastructure` / `documentation`) and a size (`size-S` / `size-M` / `size-L`). Parent issues have no size.
