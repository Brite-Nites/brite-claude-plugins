---
description: Incremental-add Flow-Driven Architecture orchestrator (whole new domain with N sub-flows) — 7 phases / 2 gates / authors the new domain's journey doc per Q47 sub-decision 2 + its personas per BC-14018
---

<!-- eval-waiver: Seven-phase incremental-add orchestrator that authors a new domain's milestone, N sub-flows, N story docs, a journey doc, and its persona docs by dispatching sub-skills (flow-inventory-add, flow-linear-scaffold, flow-doc-author, flow-journey-author, flow-persona-author, flow-index-regen) and AskUserQuestion gates; every artifact is AI-authored Linear and doc content with no fixed-right-answer, and there is no separable decide() core in the command body. -->

# /flow:add-domain

Heavier of the two incremental-add FDA orchestrators. Adds a whole new domain (1 milestone + N sub-flows + N story docs + 1 journey doc + its persona docs) under an existing FDA-shaped project. Runs **7 phases / 2 user-confirmation gates** per Q47 lock (`plugins/flow-architecture/docs/design-rationale/fda-plugin-interview.md:745`) + the BC-14018 persona-author phase: Phase 3 is a per-domain inner loop with `N=1` domain but N sub-flows inside, mirroring the Q37 hybrid control flow degenerated to N=1 domain (memory:756); Phases 4-5-6 are globally batched with N=1 domain (degenerate but consistent with the start-project sibling). Wall ≈ 10-30 min depending on sub-flow count.
...
```
  /flow:add-domain (incremental-add) — 7 phases / 2 gates
  ═══════════════════════════════════════════════════════════════════════

   ┌─ Phase 1 ─┐    ┌─ Phase 2 ─┐ Q20.6 ┌─ Phase 3 ─┐ Q13.4 ┌─ Phase 4 ─┐    ┌─ Phase 5 ─┐    ┌─ Phase 6 ─┐
   │ preflight │───►│  inventory│──────►│  linear-  │──────►│  doc-     │───►│  journey- │───►│  persona- │
   │ bootstrap │    │ add domain│       │  scaffold │       │  author   │    │   author  │    │   author  │
   │   (Q12)   │    │   (Q20    │       │   (Q13)   │       │   (Q15)   │    │   (Q16)   │    │ (BC-14018)│
   └───────────┘    │  domain-  │       └─────┬─────┘       └───────────┘    └───────────┘    └─────┬─────┘
   mode=            │   add)    │             ↓ L3 review                                           ↓
   incremental-add  └─────┬─────┘             → 5 disciplines                                  ┌─ Phase 7 ─┐
                          ↓ within-skill        per sub-flow                                  │  regen-   │
                          gate (Q20.6 owns)     (before Q13.4                                 │   index   │
                          + L2 review per       preview)                                      └─────┬─────┘
                          domain (CEO+Design,                                                       ↓
                          stashed for Phase 5)                                            status: completed
```
...
`status` transitions: `in_flight` (set at Phase 1 entry) → `completed` (Phase 7 terminator) OR `abandoned` (user halt at any gate OR Q20.4 duplicate hard-reject in Phase 2 per Q47 sub-decision 7 + user lock 2026-05-07).
...
| 6 | re-run Phase 6 (`flow-persona-author` for the new domain's personas). The skill's skip-if-exists keeps already-written persona docs from being clobbered without `--force`. |
| 7 | re-run Phase 7 — INDEX regen is idempotent. |
...
7. Emits Q20.7 `state.inventory_changed = true` flag — orchestrator captures into `state.inventory_changed` for Phase 7 INDEX regen trigger.
...
- `state.inventory_changed = false` — IMPORTANT: this branch does NOT modify inventory, so Phase 7 INDEX regen does not need a fresh trigger from this run; the existing INDEX may already render the inventory section correctly (post-Phase-7 audit will verify).
...
## Phase 6: persona author (new domain)

**Sub-skill:** `flow-persona-author` (BC-14018; shipped at `plugins/flow-architecture/skills/flow-persona-author/SKILL.md`).

This phase invokes `flow-persona-author` ONCE for the new domain. Authored AFTER the journey doc (Phase 5) because the journey is the persona-doc-author agent's richest behavioral source, and the new domain's story docs (Phase 4) are the source of the `personas:` slug set.

**Run:** dispatch `flow-persona-author` scoped to the new domain (`state.domains[0]`). The skill:

1. Reconciles the new domain's persona set — the union of every non-empty story-doc `personas:` slug across `docs/product/flows/<target_domain>/*.md` (same parse as `flow_persona_lint.py`) with the inventory persona column, minus honest-empty (ADR-041 / ADR-029 honest-empty canon).
2. Per slug, gathers the `persona-doc-author` inputs (slug, display_name, device, `journey_paths` = the new domain's journey, `served_flows` = the domain's flows that name the slug, intent_path, template_path, today).
3. Writes whole-file persona docs at `docs/product/personas/<slug>.md` (front-matter + body from the agent; only `last_reviewed` dispatcher-supplied — no builder). Strips inline HTML source-comments before writing.
4. **Skip-if-exists is the common case here** — a new domain typically reuses existing project personas; an existing persona doc is preserved (the new domain's stories cross-link to it) unless `--force` is passed. Only genuinely new personas the domain introduces are authored.
5. Authors/refreshes `docs/product/personas/INDEX.md` (new rows `Drafted`; `quality-reviewer` promotes to `Reviewed`).
6. Log + continue: a persona returning the `PERSONA-DOC-AUTHOR-ERROR` sentinel surfaces in the summary.

**Capture into state:** `state.ship_artifacts.persona_docs[]`.

**No gate.** BC-14018 locks 0 sync gates for Phase 6 (filesystem write; git review is the implicit gate — same rule as Phases 4/5).

**Breadcrumb update (end of Phase 6):** per § Phase-exit; next phase is `7`. No phase-specific delta fields.

**Failure semantics (Phase 6):** log + continue per BC-14018. Same shape as Phase 4/5 — one failed persona surfaces in the summary, never aborts; user re-runs with `--force`.
...
> **Q20 amendment 1 note (BC-9971):** on Branch B (`inventory-only` re-scaffold), `state.inventory_changed = false` because the existing inventory section is the canonical record (no append happens). Phase 7 still runs `flow-regen-index` so that the INDEX picks up the new per-sub-flow story doc rows authored in Phase 4 + the journey doc row authored in Phase 5; regen-index is idempotent so a no-op-regen against an already-correct INDEX is also safe. Branches C / D inherit whichever Q15.3 / Q16.3 path their `AskUserQuestion` choice selected.
...
### 7.1 Phase 7 terminator + final breadcrumb write
...
- `docs/product/personas/<slug>.md` per new behavioral persona the domain introduces + `docs/product/personas/INDEX.md`
...
**Failure semantics (7.1):** n/a — terminator. Any failure prior to the final breadcrumb write leaves breadcrumb at Phase 6 or earlier; resume picks up appropriately.
...
2. Set `breadcrumb.current_phase` to the next phase number (or leave at `7` after Phase 7).
3. Set `breadcrumb.status` (`in_flight` until Phase 7 terminator; then `completed`).
...
- `plugins/flow-architecture/commands/add-sub-flow.md` — sibling incremental-add orchestrator (BC-6965; 5 phases / 2 gates / skips journey-author; this command is the heavier 7-phase variant that authors the new journey doc + personas).
