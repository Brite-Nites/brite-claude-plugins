...
1. Parse `.flow/config.json` (written by flow-preflight Section 4.3) and bind the three field values to local identifiers used in step 2 — JSON keys on disk are lowercase, identifiers below are uppercase to match the flow-preflight Section 5 preamble naming convention:
