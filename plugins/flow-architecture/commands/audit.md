...
> **DO NOT re-derive** the gate manifest, hard/soft classification, override mechanism, or three-section reporting format. All seven sub-decisions of Q38 are locked at `plugins/flow-architecture/docs/design-rationale/project_fda_plugin_interview.md:713` with a refinement audit trail at `:729`. Q38 sub-decision 4's deferred-decision resolution is at `:743`. Q29's full 35-gate manifest is locked at `:240` (sub-decisions 1-7 — including Q29.6 three-section reporting format at `:270` and Q29.7 verify-docs.sh integration at `:273`) and Q29 amendment 1 (`preflight-complete` gate, locked 2026-05-11 per BC-7066) at `:275`. Surfaced via `_shared/artifact-gate-pattern.md`; this command is the runner for that manifest, not a re-statement of it.
...
```
  ...
  ═══════════════════════════════════════════════════════════════════════
   ...
   (NO override AUQ —        Override (record {gate, reason,
   only halt-on-fail in      timestamp, scope} in breadcrumb
   the pipeline)             overrides[]; counted as pass per
                             Q38 sub-decision 6) /
                             Halt (exit 1)
                                              ↓
...
The `cd $(git rev-parse --show-toplevel)` discipline pins execution to the consuming-project repo root regardless of the user's CWD when `/flow:audit` was invoked — same convention as `flow-resume-breadcrumb.sh`'s `resolve_default_path()` helper. Never invoke `bash scripts/verify-docs.sh` raw from CWD; an attacker-staged `scripts/verify-docs.sh` in a nested directory would otherwise execute under the user's shell.
...
When `--no-verify-docs` is set, the Phase B `[Story]` 5th check + `[Docs]` 4th check (which depend on the cached Phase A stdout) emit `gate.status: skipped (no Phase A cache)` rather than evaluated — they do not fall back to a synthesized pass and they do not contribute to `exit 1`.
...
- **[Story] (5 checks)** — story doc exists at `docs/product/flows/<domain>/<flow-id>.md`; required front-matter populated; job-story sentence matches regex `^> \*\*When\*\*.*\*\*I want to\*\*.*\*\*so I can\*\*`; AC has 3-5 Gherkin `Scenario:` blocks; story doc passes `verify-docs.sh` (re-checked from cached Phase A stdout).
...
**Per-flow row aggregation** happens after Phase C completes — Phase B emits its filesystem checks immediately into the report, then waits for Phase C to fold in Linear-state results before rendering the per-flow row. Phase C is where the actual `list_issues` call lives; Phase B owns the row aggregation. This is the meaning of the "intermixed" framing in the cross-cutting consistency section below.

**`--gate=<id>` valid-ID enumeration** (the canonical universe `--gate=<id>` accepts; an unrecognized value exits 64):
...
**Per-QA `list_comments` calls** (the `[QA]` 4th check) issue per-QA-child since Linear MCP has no batched-comments primitive. Fire them **in parallel** per-domain (e.g., `Promise.all`-style) with a 5s per-call soft-timeout. Sequential per-flow execution would add ~10s wall on a 50-flow project; parallelization holds total Phase C wall to the slowest single batch + slowest single `list_comments`. v1.1 parking lot: a multi-issue `list_comments` MCP tool would let this fold into the per-domain batch.

**Sandbox URL HTTP smoke-tests** (the `[Eng]` 3rd check) similarly fire per-flow. Parallelize with a 3s per-URL HTTP-HEAD timeout; cache by URL within the audit run (different flows can share a sandbox URL); transient 5xx counts as `gate: unknown` per § Failure semantics, not as hard-fail.
