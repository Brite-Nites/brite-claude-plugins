...
**Sandbox URL HTTP smoke-tests** (the `[Eng]` 3rd check) similarly fire per-flow. Parallelize with a 3s per-URL HTTP-HEAD timeout; cache by URL within the audit run (different flows can share a sandbox URL); cap in-flight requests at **8 per unique hostname** (most flows share a host even when URL paths differ; an unbounded burst against one sandbox would trigger 429 rate-limits cascading into spurious `gate: unknown`). Transient 5xx (and 429) counts as `gate: unknown` per § Failure semantics, not as hard-fail.
