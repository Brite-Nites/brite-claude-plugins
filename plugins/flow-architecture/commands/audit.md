...
- `journey-front-matter-populated` per journey doc — every `docs/product/journeys/*.md` (the all-journeys set — not just the per-domain `journeys/{domain}.md` that `journey-complete` checks) satisfies the ADR-033 journey frontmatter canon (no missing canon key, no drift key). Brings Phase B up to parity with the CI runner's journey lint so an orphan/overview journey missing a key surfaces locally too (Q29 amendment 6, BC-13935). **Filter scope:** the all-journeys sweep is the UNFILTERED behavior — the lockstep with the CI runner, which always runs unfiltered. A `--domain=<D>` run scopes this gate to the single `journey:<D>` row (the domain's own `journeys/<D>.md`); orphan/overview journeys (`journey:<stem>` whose stem matches no domain) are **out of scope for a domain-filtered run** — they surface only in the unfiltered audit + CI. So a domain-scoped `/flow:audit` / `/flow:ship` never hard-fails on an unrelated domain's or an orphan journey.
...
Phase B emits one `gate.status` entry per evaluated check into the report. Filter args (`--domain`, `--flow`, `--discipline`) gate which checks fire — unfiltered checks are simply not evaluated (not marked skipped — they are out of scope for the requested audit). For the `journey-front-matter-populated` gate (scope `journey:<stem>`), a `--domain=<D>` filter selects the `journey:<D>` row only (the domain's own `journeys/<D>.md`); orphan/overview journeys whose stem matches no domain are out of scope for a domain-filtered run and are evaluated only by the unfiltered audit (and CI, which always runs unfiltered). This keeps `/flow:ship --domain=<D>` from hard-failing on a journey outside `<D>` while preserving full all-journeys coverage in CI.
