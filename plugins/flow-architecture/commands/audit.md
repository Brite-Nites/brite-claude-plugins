---
description: Flow-Driven Architecture quality-gate runner — three-phase pipeline (verify-docs → filesystem gates → Linear MCP gates) emitting markdown or --json over Q29's 37-gate stack
---

# /flow:audit

Utility command. Single-purpose runner for the Q29 37-gate quality-gate stack (`plugins/flow-architecture/skills/_shared/artifact-gate-pattern.md`; post-Q29 amendment 6 adding the 9th phase-transition gate `journey-front-matter-populated`). Three phases / **zero user-confirmation gates between internal steps** (utility, not orchestrator) / **READ-MOSTLY** — read-only against the filesystem and Linear MCP except for the single breadcrumb `overrides[]` append on user-selected Override per Q29 sub-decision 5 (see § Override mechanism for the write contract). Default wall ≈ 14s on a 28-domain Brand-Hub-shape project (Q38 sub-decision 3 batched-list-issues optimization); ~125s without batching.
...
**The `[Story]` `story-job-story-regex` check is unconditionally human-frame-only** (Q29 amendment 4 / BC-12197 — the collapse of the per-repo `story_frame` strangler-fig once every FDA consumer reached `story_frame: strict`). Emit `FAIL story-job-story-regex` for any story doc whose frame region lacks all three human markers (`**When**` + `**I want to**` + `**so I can**`), *even if* it carries the legacy `**Given**` + `**MUST**` + `**so that**` constraint-spec markers — the constraint-spec frame is rejected, not an advisory. There is no per-repo mode any more (a leftover `story_frame` key in `.flow/config.json` is an ignored extra). The deterministic reference is `tests/run-audit-smoke.sh` `story_frame_present <doc>`.

**The `[Story]` `story-front-matter-populated` check requires the full canon unconditionally** (BC-13915 — the collapse of the per-repo `frontmatter_schema` strangler-fig once every FDA consumer reached `frontmatter_schema: strict`). It enforces the **full 20-key story canon** (ADR-029 + the job-story template), **presence not non-emptiness** (honest-empty `personas: []` passes) — and the `redirect-front-matter-valid` + journey-frontmatter checks are likewise unconditional. A drift key (`sub_flow_id`, `linear_parent`) fails this gate only via the canonical key it displaces going absent — *naming* the drift is the standalone `scripts/flow-frontmatter-lint.sh` (A-10/A-11), not this completeness gate. There is no per-repo mode any more (a leftover `frontmatter_schema` key in `.flow/config.json` is an ignored extra). The deterministic reference is `tests/run-audit-smoke.sh` `story_frontmatter_populated <doc>`, which single-sources the canon from `scripts/lib/flow_frontmatter_lint.py`.
...
- `journey-front-matter-populated` per journey doc — every `docs/product/journeys/*.md` (the all-journeys set — not just the per-domain `journeys/{domain}.md` that `journey-complete` checks) satisfies the ADR-033 journey frontmatter canon (no missing canon key, no drift key). Brings Phase B up to parity with the CI runner's journey lint so an orphan/overview journey missing a key surfaces locally too (Q29 amendment 6, BC-13935). **Scope + filter:** a per-domain journey (`journeys/<D>.md`, `<D>` a real domain) emits scope **`domain:<D>`** — identical to `journey-complete` for the same file — so a `--domain=<D>` audit filters it mechanically by scope-prefix (exactly as the other per-domain gates). A genuine orphan/overview journey whose stem matches no domain emits scope **`journey:<stem>`**, a project-level row only the UNFILTERED audit + CI surface (CI always runs unfiltered — the lockstep with the runner). So a domain-scoped `/flow:audit` / `/flow:ship` never hard-fails on an unrelated domain's or an orphan journey, while CI retains full all-journeys coverage.
...
- **[Story] (5 checks)** — story doc exists at `docs/product/flows/<domain>/<flow-id>.md`; **front-matter populated (unconditionally the full 20-key story canon, presence-not-emptiness — BC-13915; see the canon note above)**; **story-frame present (unconditionally human-frame-only, BC-12197; line-form-agnostic per rubric D11)** — within the **story-frame region** (the `## Job story` section, or — for the leaner heading-less docs — the title + summary blockquote up to the first `## Acceptance` heading) the doc carries the human job-story markers (`**When**` + `**I want to**` + `**so I can**`). The **legacy constraint-spec markers** (`**Given**` + `**MUST**` + `**so that**`, a non-human / infrastructure actor — crawler, sitemap/robots, cron, webhook, CDN, ISR, canonical, CSP) **do NOT satisfy the gate**: a constraint-spec-only doc FAILs and must be re-anchored on the human the mechanism serves. The check is **region-scoped** (title → first `## Acceptance`), so the markers may span one line OR the canonical brite-base multi-line blockquoted form (`> **When** …` / `> **I want to** …` / `> **so I can** …`); a single-line regex would reject the multi-line GOLD format. AC has 3-5 Gherkin `Scenario:` blocks; story doc passes `verify-docs.sh` (re-checked from cached Phase A stdout). _(The `story-job-story-regex` gate ID is unchanged for Q29 gate-stack stability. The human-only frame was locked progressively: BC-12134 retired the constraint-spec frame from generators; **Q29 amendment 3** (BC-11983) added a per-repo `story_frame: strict` gate-narrowing strangler-fig; **Q29 amendment 4** (BC-12197) collapsed that flag to the hardcoded human-only global end-state once all FDA consumers reached `strict`. The deterministic mirror is `tests/run-audit-smoke.sh` `story_frame_present <doc>`. Line-form-agnostic since BC-11988 / T0-4.)_
...
Phase B emits one `gate.status` entry per evaluated check into the report. Filter args (`--domain`, `--flow`, `--discipline`) gate which checks fire — unfiltered checks are simply not evaluated (not marked skipped — they are out of scope for the requested audit). The `journey-front-matter-populated` gate emits scope `domain:<D>` for a per-domain journey (`journeys/<D>.md` — mirroring `journey-complete`) and `journey:<stem>` for an orphan/overview journey; a `--domain=<D>` filter selects the `domain:<D>` row by scope-prefix (exactly as for the other per-domain gates), and orphan journeys are out of scope for a domain-filtered run — evaluated only by the unfiltered audit (and CI, which always runs unfiltered). This keeps `/flow:ship --domain=<D>` from hard-failing on a journey outside `<D>` while preserving full all-journeys coverage in CI.
...
| Category | Stable IDs |
|---|---|
| Phase-transition (Q29.1) | `env-ready`, `preflight-complete`, `intent-exists`, `inventory-complete`, `scaffold-complete`, `story-docs-complete`, `journey-complete`, `journey-front-matter-populated`, `index-complete` |
...
**CI-runner resolution gates (`link-resolution` + `persona-exists`).** Two deterministic cross-doc *resolution* gates are enforced by the continuous CI runner (`scripts/run_fda_ci_audit.py`, the headless surface the consumer `fda-audit` required check invokes) but are **not** emitted as discrete per-flow Phase-B rows by the interactive `/flow:audit` evaluator, so they are absent from the `--gate` universe above: (a) **`link-resolution`** — every body `](<path>.md)` link resolves on disk (BC-13710); (b) **`persona-exists`** — every NON-EMPTY story-doc `personas:` slug resolves to `docs/product/personas/<slug>.md` (BC-12573, the deterministic FLOOR of the 3-layer persona system; **activated fleet-wide by BC-14036** once every consumer was migrated clean). Honest-empty (`personas: []` / absent / null) passes — presence, not non-emptiness (ADR-029); `personas:` is behavioral persona-doc slugs only and an RBAC/access overload is an off-canon FAIL, not a carve (ADR-041). Both are single-sourced from their lint modules (`flow_persona_lint` for persona-exists — the SAME definition the `validate.sh` fixture vslice locks), so CI and the standalone lints never drift.
...
- `plugins/flow-architecture/docs/design-rationale/fda-plugin-interview.md:240` — Q29 35-gate manifest lock (pre-amendment-2; canonical post-amendment total is 37 per Q29 amendment 6).
