...
**Resolve the per-repo `story_frame` mode once, up front** (Q29 amendment 3): read the `story_frame` field from the already-loaded `.flow/config.json` (the same file the `preflight-complete` gate reads). The mode is `strict` only when the value is the string `strict` (case-insensitive); anything else — field absent, unrecognized value, or no config — resolves **fail-safe to `lenient`**. Apply this mode to the `[Story]` `story-job-story-regex` check below (and only that check) as a **hard verdict**: under `strict`, emit `FAIL story-job-story-regex` for any story doc whose frame region lacks all three human markers (`**When**` + `**I want to**` + `**so I can**`), *even if* it carries the legacy `**Given**` + `**MUST**` + `**so that**` constraint-spec markers — it is a gate failure, not an advisory. The deterministic reference is `tests/run-audit-smoke.sh` `story_frame_mode <repo>` → `story_frame_present <doc> <mode>`.

**Resolve the per-repo `frontmatter_schema` mode the same way** (BC-12572): read the `frontmatter_schema` field from the same `.flow/config.json`; `strict` only for the string `strict` (case-insensitive), everything else **fail-safe `lenient`**. Apply it to the `[Story]` `story-front-matter-populated` check below (and only that check): under `lenient` (default) the check is the 4-key presence floor (`flow_id`/`status`/`figma`/`user_docs_url`) — today's behavior; under `strict` it widens to the **full 20-key story canon** (ADR-029 + the job-story template), still **presence not non-emptiness** (honest-empty `personas: []` passes). A drift key (`sub_flow_id`, `linear_parent`) fails this gate only via the canonical key it displaces going absent — *naming* the drift is the standalone `scripts/flow-frontmatter-lint.sh` (A-10/A-11), not this completeness gate. The deterministic reference is `tests/run-audit-smoke.sh` `frontmatter_schema_mode <repo>` → `story_frontmatter_populated <doc> <mode>`, which single-sources the canon from `scripts/lib/flow_frontmatter_lint.py`. Same transient strangler-fig as `story_frame` — repos flip to `strict` as their docs migrate to full canon; deleted when all converge.
...
- **[Story] (5 checks)** — story doc exists at `docs/product/flows/<domain>/<flow-id>.md`; **front-matter populated (per-repo schema strictness via `.flow/config.json` `frontmatter_schema`: `lenient` default = the 4-key floor `flow_id`/`status`/`figma`/`user_docs_url`; `strict` = the full 20-key story canon, presence-not-emptiness — see the mode-resolution note above)**; **story-frame present (per-repo frame strictness via `.flow/config.json` `story_frame`; line-form-agnostic per rubric D11)** — within the **story-frame region** (the `## Job story` section, or — for the leaner heading-less docs — the title + summary blockquote up to the first `## Acceptance` heading) the doc carries the human job-story markers (`**When**` + `**I want to**` + `**so I can**`). Whether the **legacy constraint-spec markers** (`**Given**` + `**MUST**` + `**so that**`, a non-human / infrastructure actor — crawler, sitemap/robots, cron, webhook, CDN, ISR, canonical, CSP) *also* satisfy the gate depends on the consuming repo's `story_frame` setting: **`lenient` (default — field absent)** accepts them too (the BC-12134 presence-floor, so not-yet-reframed repos keep passing); **`strict`** rejects them, so a constraint-spec-only doc FAILs and must be re-anchored on the human the mechanism serves. The check is **region-scoped** (title → first `## Acceptance`), so the markers may span one line OR the canonical brite-base multi-line blockquoted form (`> **When** …` / `> **I want to** …` / `> **so I can** …`) in either mode; a single-line regex would reject the multi-line GOLD format. AC has 3-5 Gherkin `Scenario:` blocks; story doc passes `verify-docs.sh` (re-checked from cached Phase A stdout). _(The `story-job-story-regex` gate ID is retained across both modes for Q29 gate-stack stability — the accepted-frame-set is config-gated, not a new gate ID, same discipline as the BC-12134 lenient widening. Per-repo `story_frame: strict` is the gate-narrowing locked at **Q29 amendment 3** (BC-11983): it is a TRANSIENT strangler-fig — repos flip to `strict` as their story docs are reframed; once all 7 WS-E consumer repos are `strict` the flag is removed and `strict` is hardcoded as the global end-state. The deterministic mirror of this two-mode behavior is `tests/run-audit-smoke.sh` `story_frame_present <doc> <mode>` + `story_frame_mode <repo>`. Line-form-agnostic since BC-11988 / T0-4.)_
