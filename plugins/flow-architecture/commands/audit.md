...
- **[Story] (5 checks)** — story doc exists at `docs/product/flows/<domain>/<flow-id>.md`; required front-matter populated; **story-frame present (frame-agnostic + line-form-agnostic per rubric D11)** — within the **story-frame region** (the `## Job story` section, or — for the leaner heading-less docs — the title + summary blockquote up to the first `## Acceptance` heading) the doc carries EITHER the human job-story markers (`**When**` + `**I want to**` + `**so I can**`) OR — tolerated for backward-compat with not-yet-reframed consumer repos — the legacy constraint-spec markers (`**Given**` + `**MUST**` + `**so that**`) a non-human / infrastructure actor (crawler, sitemap/robots, cron, webhook, CDN, ISR, canonical, CSP) once used. The check is **region-scoped** (title → first `## Acceptance`), so the markers may span one line OR the canonical brite-base multi-line blockquoted form (`> **When** …` / `> **I want to** …` / `> **so I can** …`); a single-line regex would reject the multi-line GOLD format. AC has 3-5 Gherkin `Scenario:` blocks; story doc passes `verify-docs.sh` (re-checked from cached Phase A stdout). _(The `story-job-story-regex` gate ID is retained for Q29 gate-stack stability. It stays **frame-agnostic** as a permissive presence-floor: it still accepts the legacy constraint-spec frame so consumer repos not yet reframed keep passing during the human-anchoring migration (BC-12134), even though the human JTBD frame is now the canonical target per rubric D11 — gate-narrowing is deferred until all consumers are reframed. Line-form-agnostic since BC-11988 / T0-4.)_
