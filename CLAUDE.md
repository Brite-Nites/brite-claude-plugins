...
- [ADR-035: Campaign-lifecycle EB-draft staging boundary](docs/decisions/035-campaign-lifecycle-eb-draft-staging-boundary.md) — re-cut plan-campaign/launch-campaign so plan-campaign stages the whole vessel **incl. the EB draft** (copy via `email-copywriting` sub-phase + ESP-only triplet Google/Microsoft/SMTP + senders, no leads) and launch-campaign becomes **leads-only**. ESP split is a sender-side partition (EB has no native recipient-ESP routing — verified), so it's plan-time-knowable; email-type stays recipient-derived at attach-time. Draft campaigns reuse ADR-020's `email_bison.campaigns[]` (one `status:draft` record per `esp` google/microsoft/smtp — NOT a new dict); EB writes are command-side IO (ADR-028 builder boundary preserved); double idempotency (milestone + drafts), EB phase soft-fails like SF. **Absorbs the deferred BC-11857 v2-scaffolder pin** (build_manifest.py still emits v1). Open: `audience_tier.tier` at plan time (BC-13628)
