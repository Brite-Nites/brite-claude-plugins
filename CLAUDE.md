...
- **Pre-tool-use security hook can block content-level authoring, not just shell commands.** Writing a Skill/Command file that enumerates specific internal domain names, credential env-var names (e.g. `FOO_API_TOKEN`), or attack-payload examples will trigger the hook's `{"ok": false}` response even when those values aren't actual secrets. Workaround: reference external allowlist/config files and use generic phrasing ("any credential env var", "the documented allowlist at `<path>`") rather than inline enumeration. First observed in BC-5826 § Input validation authoring.
