...
- [ADR-030: Snowflake access for marketing plugin](docs/decisions/030-marketing-snowflake-access.md) — `snow` CLI wrapper script via `bw-run.sh` (Option B); rejects Snowflake MCP (Option A, MCP-cap pressure) and brite-data-platform HTTP gateway (Option C, no service exists)
- [ADR-031: Anti-spam guardrails consolidation](docs/decisions/031-anti-spam-guardrails.md) — five TAM-protection rules (180-day non-repeat, 2-cycle/year, enterprise-commercial-post-Q2, Tier-1 Q1-Q2 window, calendar-year offer rotation) owned by `prospect-temporal-gate`
