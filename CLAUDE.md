...
- **`SessionStart` `command`-hook stdout is model-context-only in Claude Code v2.1.0+** — hooks fire correctly and their stdout reaches the model via `<system-reminder>` (skill activation works), but the content no longer renders as a visible terminal block above the first prompt. Tracked upstream as [#24425](https://github.com/anthropics/claude-code/issues/24425). For user-visible session context, Anthropic's documented channel is statusline (`statusLine` setting) or a user-invoked slash command — not hooks. Diagnosed in BC-6434.
