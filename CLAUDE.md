...
- [ADR-032: GTM canonical Discovery ICP layer](docs/decisions/032-gtm-canonical-discovery-icp.md) — per-vertical `data/canonicals/icp/{vertical}.json` (segments-only, persona cross-ref, mandatory + lint-enforced); plan-campaign single-source ICP resolution + per-segment campaign criteria copies (BC-11163)
