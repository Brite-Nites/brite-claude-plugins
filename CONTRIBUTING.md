...
- **Vault-lock-mid-session UX:** ~30s recovery cost per lock event in restart-based env-propagation mode (BC-5947 task-3 Pattern A; BC-6905 Q5 measurement). Aggressive idle-lock policies compound this — document expected unlock cadence in onboarding. **Opt-in escape hatch (2026-07-29):** provision the macOS Keychain item `bw-master` (`security add-generic-password -U -a "$USER" -s bw-master -w`) and the wrapper self-unlocks on absent/stale sessions — lock events stop costing anything, and MCP spawns no longer depend on the launching shell having `BW_SESSION` exported. Trust trade-off recorded in ADR-010 § 1.
