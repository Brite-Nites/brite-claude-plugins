...
if [ ! -f "$structural_lint_test" ]; then
  ...
  if sl_out=$(python3 "$structural_lint" --scan-repo 2>&1); then
    ...
    if [ "${sl_total:-0}" -eq 0 ]; then
      ...
      warn "structural lint — advisory findings never fail the build; gate-tier findings are BLOCKING via eval_gate (diff-gate + --structural, §15a-bc-12590 Part 3 / ADR-034): $sl_summary"
...
#   3. The full-surface structural gate (`eval_gate.py --structural`, ADR-034 /
...
if [ ! -f "$eval_gate" ]; then
  fail "scripts/eval/eval_gate.py not found — the ADR-034 full-surface structural gate cannot run"
...
section "15a-bc-17213. Lead-CSV column mapper harness (BC-17213)"

cm_harness="$REPO_ROOT/plugins/marketing/scripts/test_column_map.sh"

if [ ! -f "$cm_harness" ]; then
  warn "plugins/marketing/scripts/test_column_map.sh not found — column mapper harness skipped"
else
  if cm_harness_out=$(bash "$cm_harness" 2>&1); then
    cm_pass_count=$(printf '%s\n' "$cm_harness_out" | sed -n 's/^RESULT pass=\([0-9][0-9]*\) fail=.*/\1/p' | tail -1)
    if [ -n "$cm_pass_count" ]; then
      pass "lead-CSV column mapper harness (${cm_pass_count} assertions)"
    else
      pass "lead-CSV column mapper harness — passed (count unparsed)"
    fi
  else
    fail "lead-CSV column mapper harness failed:"
    printf '%s\n' "$cm_harness_out" | tail -30 | sed 's/^/          /' >&2
  fi
fi
...
# build_canonical_emit.py, the hermetic emit harness the canonicals bootstrap command
# family (/marketing:new-offer, new-persona, new-vertical) behavioral evals drive (ADR-028
# STRUCTURE-FIRST / LLM-judged representatives). The builder does NOT re-implement any
# logic — it shells the SAME runtime entrypoints the commands delegate to
# (canonicals_bootstrap.py `<subcommand>`, which OWNS every input guard, +
# lint_canonicals.py, the 19-check ADR-016 contract) against a sandbox copy of a FROZEN
# seed. The suite drives every verdict branch across all three subcommands, proves a
# scenario value is NEVER shelled (a `$(touch pwned)` display reaches the entry as inert
# data — no `pwned` sentinel), and locks determinism + infra exit codes. FAIL-if-missing
# (not warn): both the commands' evals and the structure-first proof depend on it, so a
# future delete must fail loudly (the §15a-bc-12589 lesson). RESULT line drives the count.
# ══════════════════════════════════════════════════════════════════════
section "15a-bc-12702. canonicals-family emit builder unit suite (BC-12702 + BC-12915)"

no_helper="$REPO_ROOT/plugins/marketing/scripts/build_canonical_emit.py"
no_harness="$REPO_ROOT/plugins/marketing/scripts/test_canonical_emit.sh"

if [ ! -f "$no_helper" ]; then
  fail "plugins/marketing/scripts/build_canonical_emit.py not found — the canonicals emit-mode builder is missing"
elif [ ! -f "$no_harness" ]; then
  fail "plugins/marketing/scripts/test_canonical_emit.sh not found — the canonicals builder suite is missing"
else
